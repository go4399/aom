...
static void grain_table_entry_read(FILE *file,
  ...
  int num_read =
      fscanf(file, "E %%%" PRId64 " %%%" PRId64 " %d %hd %d\n",
             &entry->start_time, &entry->end_time, &pars->apply_grain,
             &pars->random_seed, &pars->update_parameters);
...
aom_codec_err_t aom_film_grain_table_read(
  ...
  if (setjmp(error_info->jmp) == 0) {
    ...
    // the header.
    char magic[9];
    if (!fread(magic, 9, 1, (FILE *)file) || memcmp(magic, kFileMagic, 8)) {
      aom_internal_error(error_info, AOM_CODEC_ERROR,
    ...
    for (;;) {
      ...
      grain_table_entry_read((FILE *)file, error_info, entry);
      entry->next = NULL;
...
aom_codec_err_t aom_film_grain_table_write(
  ...
  if (setjmp(error_info->jmp) == 0) {
    ...
    if (!fwrite(kFileMagic, 8, 1, (FILE *)file)) {
      aom_internal_error(error_info, AOM_CODEC_ERROR,
    ...
    aom_film_grain_table_entry_t *entry = t->head;
    while (entry) {
      grain_table_entry_write((FILE *)file, entry);
      entry = entry->next;
    }
    fclose((FILE *)file);
    file = NULL;
