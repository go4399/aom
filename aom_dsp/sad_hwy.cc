  ...
  for (int i = 0; i < h; ++i) {
    for (int j = 0; j < BlockWidth; j += vw) {
      ...
      if (second_pred) {
        ...
        auto ref2_vec = hn::AverageRound(ref_vec, sec_pred_vec);
        sad = hn::SumsOf8AbsDiff(src_vec, ref2_vec);
      } else {
...
HWY_MAYBE_UNUSED void SumOfAbsoluteDiff4D(const uint8_t *src_ptr,
  ...
  constexpr hn::BlockDFromD<decltype(uint32_tag)> interleave_tag;
  ...
  constexpr hn::CappedTag<uint32_t, 4> block_sum_tag;
