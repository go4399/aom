...
HWY_INLINE unsigned int SumOfAbsoluteDiff64xN(const uint8_t *src_ptr,
  ...
  for (int i = 0; i < h; i++) {
    ...
    hn::Print(pixel_tag, "ref_vec_1", ref_vec_1, 0, 32);
    auto ref_vec_2 = hn::LoadU(pixel_tag, ref_ptr + 32);
    auto sad_1 = hn::AbsDiff(ref_vec_1, hn::LoadU(pixel_tag, src_ptr));
    auto sad_2 = hn::AbsDiff(ref_vec_2, hn::LoadU(pixel_tag, src_ptr + 32));
    sum_sad = hn::Add(sum_sad, hn::Add(sad_1, sad_2));
    hn::Print(pixel_tag, "sum_sad", sum_sad, 0, 32);
    unsigned int res_1 = hn::ReduceSum(pixel_tag, sum_sad);
    int sum_c = 0;
    for (int j = 0; j < 32; j++) {
      sum_c += std::abs(src_ptr[j] - ref_ptr[j]);
    }
    src_ptr += src_stride;
    ref_ptr += ref_stride;
  }
