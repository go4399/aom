...
static int parse_chroma_format_bitdepth(struct aom_read_bit_buffer *reader,
  ...
  int subsampling_x;
  int subsampling_y;
  aom_codec_err_t err = av2_get_chroma_subsampling(
...
static int parse_content_intrepretation_obu(const uint8_t *const buffer,
  ...
  if (ci_color_description_present_flag) {
    ...
    if (config->ci_color_description_idc == AV2_COLOR_DESC_IDC_EXPLICIT) {
      ...
      config->ci_color_primaries = AOM_CICP_CP_UNSPECIFIED;
      config->ci_transfer_characteristics = AOM_CICP_TC_UNSPECIFIED;
      config->ci_matrix_coefficients = AOM_CICP_MC_UNSPECIFIED;
    }

    AV2C_READ_BIT_OR_RETURN_ERROR(full_range_flag);
    config->ci_full_range_flag = full_range_flag;
  } else {
    config->ci_color_description_idc = AV2_COLOR_DESC_IDC_EXPLICIT;
    config->ci_color_primaries = AOM_CICP_CP_UNSPECIFIED;
    config->ci_transfer_characteristics = AOM_CICP_TC_UNSPECIFIED;
    config->ci_matrix_coefficients = AOM_CICP_MC_UNSPECIFIED;
...
int write_av2config(const Av2Config *config, size_t capacity,
  ...
  *bytes_written = 0;
  ...
  aom_wb_write_literal(&writer, config->seq_profile_idc, PROFILE_BITS);
