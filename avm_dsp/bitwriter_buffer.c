...
// Scoped overrides for static bitwriter_buffer helpers to prevent collision
#define wb_write_primitive_quniform avm_wb_write_primitive_quniform_static
#define wb_write_primitive_subexpfin avm_wb_write_primitive_subexpfin_static
...
void avm_wb_write_literal(struct avm_write_bit_buffer *wb, int data, int bits) {
  assert(bits <= 31);
  int bit;
  for (bit = bits - 1; bit >= 0; bit--) avm_wb_write_bit(wb, (data >> bit) & 1);
}
...
void avm_wb_write_uvlc(struct avm_write_bit_buffer *wb, uint32_t v) {
  ...
  avm_wb_write_literal(wb, 0, leading_zeroes);
  avm_wb_write_unsigned_literal(wb, v, leading_zeroes + 1);
}
...
void avm_wb_write_svlc(struct avm_write_bit_buffer *wb, int32_t v) {
  ...
  if (v <= 0) {
    ...
    avm_wb_write_uvlc(wb, 2 * abs_val);
  } else {
    const uint32_t abs_val = (uint32_t)v;
    avm_wb_write_uvlc(wb, 2 * abs_val - 1);
  }
}
...
void avm_wb_write_primitive_ref_quniform(struct avm_write_bit_buffer *wb,
  ...
  avm_wb_write_bit(wb, unequal);
  if (unequal) {
    v -= (v > r);
    avm_wb_write_primitive_quniform(wb, n - 1, v);
  }
}
...
static void wb_write_primitive_subexpfin(struct avm_write_bit_buffer *wb,
  ...
  while (1) {
    ...
    if (n <= mk + 3 * a) {
      ...
      int t = (v >= mk + a);
      avm_wb_write_bit(wb, t);
      if (t) {
        i = i + 1;
        mk += a;
      } else {
        avm_wb_write_literal(wb, v - mk, b);
        break;
      }
    }
  }
...
void avm_wb_write_uleb(struct avm_write_bit_buffer *wb, uint32_t value) {
  ...
  for (size_t i = 0; i < leb_size; ++i) {
    ...
    avm_wb_write_literal(wb, encoded_byte, 8);
  }
}
...
void avm_wb_write_rice_golomb(struct avm_write_bit_buffer *wb, uint32_t data,
  ...
  avm_wb_write_unsigned_literal(wb, remainder, k);
}
