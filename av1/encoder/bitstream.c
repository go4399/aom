...
static inline void encode_loopfilter(AV1_COMMON *cm,
  ...
  if (lf->mode_ref_delta_enabled) {
    // If meaningful is true, write out the actual loop filter deltas.
    // If mode_ref_delta_enabled is true, signal whether the deltas are
    // different from the reference frame's defaults/values.
