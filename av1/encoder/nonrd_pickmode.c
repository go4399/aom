    ...
    *ref_frame2 = NONE_FRAME;
    if (is_one_pass_realtime_lag(cpi)) {
      ...
      *ref_frame = ref_mode_set_full[idx].ref_frame;
      if ((*ref_frame == BWDREF_FRAME || *ref_frame == ALTREF2_FRAME ||
           *ref_frame == LAST2_FRAME || *ref_frame == LAST3_FRAME) &&
          *this_mode != GLOBALMV)
        return true;
    } else {
