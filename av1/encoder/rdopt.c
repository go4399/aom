...
static int64_t rd_pick_intrabc_mode_sb(const AV1_COMP *cpi, MACROBLOCK *x,
  ...
  if (!av1_allow_intrabc(cm) || !cpi->oxcf.kf_cfg.enable_intrabc ||
    ...
    return INT64_MAX;
  if (cpi->sf.mv_sf.intrabc_search_level >= 1 &&
      !(bsize == BLOCK_4X4 || bsize == BLOCK_8X8 || bsize == BLOCK_16X16)) {
    return INT64_MAX;
  }
  const int num_planes = av1_num_planes(cm);
  ...
  for (enum IntrabcMotionDirection dir = IBC_MOTION_ABOVE; dir < max_dir;
    ...
    if (bestsme == INT_MAX || cpi->sf.mv_sf.intrabc_search_level == 0) {
      int pixelsme = av1_full_pixel_search(start_mv, &fullms_params, step_param,
                                           NULL, &best_search_mv.as_fullmv,
                                           &best_mv_stats, NULL);
      {
        if (pixelsme < bestsme) {
          bestsme = pixelsme;
          best_mv = best_search_mv;
        }
      }
    }
