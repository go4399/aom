...
static inline void increase_motion_mode_rdstats(const MB_MODE_INFO *this_mbmi,
  ...
  if (this_mbmi->motion_mode == WARPED_CAUSAL) {
    ...
    scale_rdstats(rd_stats, rd_warp_bias_scale);
    scale_rdstats(rd_stats_y, rd_warp_bias_scale);
    scale_rdstats(rd_stats_uv, rd_warp_bias_scale);
  } else if (this_mbmi->motion_mode == OBMC_CAUSAL) {
    const double rd_obmc_bias_scale = rd_obmc_bias_scale_pct / 100.0;
    scale_rdstats(rd_stats, rd_obmc_bias_scale);
    scale_rdstats(rd_stats_y, rd_obmc_bias_scale);
    scale_rdstats(rd_stats_uv, rd_obmc_bias_scale);
  }
}
