...
static void adjust_rdcost(const AV1_COMP *cpi, const MACROBLOCK *x,
  ...
  if ((cpi->oxcf.tune_cfg.tuning == AOM_TUNE_IQ ||
      ...
      is_inter_pred) {
    // Tune IQ and SSIMULACRA2 are often used in use cases where keyframes can
    // be encoded at a lower quality (i.e. higher QP) than inter-coded frames.
    ...
    // increasing overall efficiency, while still letting the encoder be able
    // to pick inter prediction when it's beneficial.
    rd_cost->dist = (rd_cost->dist * 9) / 8;
    rd_cost->rdcost = (rd_cost->rdcost * 9) / 8;
    return;
  }
...
static inline void search_intra_modes_in_interframe(
  ...
  *mbmi = best_mbmi;
  ...
  adjust_rdcost(cpi, x, &intra_rd_stats, false);
