...
int av1_encode(AV1_COMP *const cpi, uint8_t *const dest, size_t dest_size,
  ...
  // Check if this is the last frame in the gop
  if (gf_group->update_type[cpi->gf_frame_index] != OVERLAY_UPDATE &&
      gf_group->update_type[cpi->gf_frame_index] !=
          INTNL_OVERLAY_UPDATE) {
    int is_last = 1;
    for (int i = 0; i < gf_group->size; ++i) {
      if (gf_group->display_idx[i] > (int64_t)current_frame->display_order_hint) {
        is_last = 0;
        break;
      }
    }
    if (is_last) {
      ...
      cpi->ppi->tpl_data.prev_gop_arf_disp_order = current_frame->display_order_hint;
    }
  }
