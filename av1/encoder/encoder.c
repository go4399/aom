...
// Macro that helps debug the screen content detection mode 2 mechanism
...
void av1_set_screen_content_options(AV1_COMP *cpi, FeatureFlags *features) {
  ...
  if (cpi->oxcf.algo_cfg.sct_detection_mode == AOM_SCREEN_DETECTION_STANDARD) {
    estimate_screen_content(cpi, features);
  } else if (cpi->oxcf.algo_cfg.sct_detection_mode ==
             AOM_SCREEN_DETECTION_ANTIALIAS_AWARE) {
