 ...
 */
uint8_t av1_find_dominant_value(const uint8_t *src, ptrdiff_t stride, int rows,
                                int cols) {
    ...
    for (int c = 0; c + BLK_W <= width; c += BLK_W) {
      ...
      if (number_of_colors > 1 &&
        ...
        if (number_of_colors <= simple_color_thresh) {
          ...
          av1_dilate_block(blk, blk_stride, dilated_blk, BLK_W, BLK_W, BLK_H);
...
void av1_set_screen_content_options(AV1_COMP *cpi, FeatureFlags *features) {
  ...
  } else if (cpi->oxcf.algo_cfg.screen_detection_mode ==
