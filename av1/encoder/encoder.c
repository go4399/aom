...
static void estimate_screen_content_antialiasing_aware(AV1_COMP *cpi,
  ...
  const bool fast_detection_mode =
      cpi->sf.hl_sf.screen_detection_mode2_fast_detection_mode;
  const AV1_COMMON *const cm = &cpi->common;
  const MACROBLOCKD *const xd = &cpi->td.mb.e_mbd;
  const uint8_t *src = cpi->unfiltered_source->y_buffer;
  assert(src != NULL);
  ...
  if (fast_detection_mode) {
    fprintf(stats_file, "Fast detection mode enabled\n");
  }
  fprintf(stats_file,
          "---------------------------------------------------------------\n");
  fprintf(stats_file,
  ...
  // Skip every other block and weigh each block twice as much in fast detection
  // mode
  const int multiplier = fast_detection_mode ? 2 : 1;

  for (int r = 0; r + kBlockHeight <= height; r += kBlockHeight) {
    // Alternate skipping in a "checkerboard" pattern in fast detection mode
    const int initial_col =
        (fast_detection_mode && (r / kBlockHeight) % 2) ? kBlockWidth : 0;
