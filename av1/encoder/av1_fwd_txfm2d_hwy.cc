...
#define HWY_TARGET_INCLUDE "av1/encoder/av1_fwd_txfm2d_hwy.cc"
...
constexpr int RectangularTransformLogRatio() {
  if constexpr (Col == Row) {
    return 0;
  } else if constexpr (Col > Row) {
    if constexpr (Col == Row * 2) {
      return 1;
    } else if constexpr (Col == Row * 4) {
      return 2;
    } else if constexpr (Col == Row * 8) {
      return 3;
    } else if constexpr (Col == Row * 16) {
      return 4;
    } else {
      static_assert(false, "Unsupported column/row combination (Col > Row)");
    }
  } else {
    if constexpr (Row == Col * 2) {
      return -1;
    } else if constexpr (Row == Col * 4) {
      return -2;
    } else if constexpr (Row == Col * 8) {
      return -3;
    } else if constexpr (Row == Col * 16) {
      return -4;
    } else {
      static_assert(false, "Unsupported column/row combination (Col < Row)");
    }
...
template <typename D>
HWY_ATTR HWY_INLINE auto SetPair(D int_tag, int a, int b) {
  return hn::BitCast(
...
template <typename D>
hn::Vec2<D> ButterflyConstant(D int_tag, int w0, int w1) {
  if constexpr (sizeof(hn::TFromD<D>) == 2) {
    return hn::Create2(int_tag, SetPair(int_tag, w0, w1),
                       SetPair(int_tag, w1, -w0));
  } else {
    return hn::Create2(int_tag, hn::Set(int_tag, w0), hn::Set(int_tag, w1));
  }
}
...
template <typename D>
HWY_ATTR HWY_INLINE void Butterfly(
    D int_tag, hn::Vec2<D> w, const hn::TFromD<D> *HWY_RESTRICT in0,
    const hn::TFromD<D> *HWY_RESTRICT in1, hn::TFromD<D> *HWY_RESTRICT out0,
    hn::TFromD<D> *HWY_RESTRICT out1, int bit,
    hn::VFromD<hn::Repartition<int32_t, D>> round) {
  if constexpr (sizeof(hn::TFromD<D>) == 2) {
    constexpr hn::RepartitionToWide<D> int32_tag;
    const auto ww0 = hn::Get2<0>(w);
    const auto ww1 = hn::Get2<1>(w);
    const auto i0 = hn::Load(int_tag, in0);
    const auto i1 = hn::Load(int_tag, in1);
    const auto t0 = hn::InterleaveLower(int_tag, i0, i1);
    const auto t1 = hn::InterleaveUpper(int_tag, i0, i1);
...
HWY_ATTR HWY_INLINE hn::VFromD<D> ScaleByNewSqrt2(D tag, hn::VFromD<D> v) {
  if constexpr (sizeof(hn::TFromD<D>) == 2) {
    if constexpr (hn::MaxLanes(tag) == 4 && HWY_TARGET != HWY_EMU128) {
      auto one = hn::Set(tag, 1);
      auto scale_rounding =
...
HWY_ATTR HWY_INLINE auto RoundShift(D int_tag, hn::VFromD<D> value) {
  if constexpr (Bit == 0) {
    return value;
  } else if constexpr (Bit > 0) {
...
HWY_ATTR HWY_INLINE auto RectScale(D int_tag, hn::VFromD<D> v) {
  if constexpr (ApplyRectScale) {
...
HWY_ATTR HWY_INLINE void Transpose4x4(const TIn *HWY_RESTRICT in,
  ...
  if constexpr (std::is_same_v<TIn, TOut>) {
    hn::StoreInterleaved4(i0, i1, i2, i3, int_tag, interleaved);
  } else {
    constexpr hn::FixedTag<TOut, 4> out_tag;
    hn::StoreInterleaved4(hn::PromoteTo(out_tag, i0),
                          hn::PromoteTo(out_tag, i1),
                          hn::PromoteTo(out_tag, i2),
                          hn::PromoteTo(out_tag, i3), out_tag, interleaved);
  }
  for (size_t i = 0; i < 4; ++i) {
...
template <int8_t Bit, bool ApplyRectScale, typename TIn, typename TOut>
HWY_ATTR HWY_INLINE void Transpose8x8(const TIn *HWY_RESTRICT in,
                                      TOut *HWY_RESTRICT out, size_t instride,
                                      size_t outstride) {
  constexpr hn::FixedTag<TIn, 8> int_tag;
  constexpr hn::Rebind<TOut, decltype(int_tag)> out_tag;
  // N.B. there isn't a StoreInterleaved8, so hand-code Transpose8x8.
  ...
  if constexpr (std::is_same_v<TIn, TOut>) {
    s0 = RectScale<ApplyRectScale>(int_tag, RoundShift<Bit>(int_tag, i0));
    s1 = RectScale<ApplyRectScale>(int_tag, RoundShift<Bit>(int_tag, i1));
    s2 = RectScale<ApplyRectScale>(int_tag, RoundShift<Bit>(int_tag, i2));
    s3 = RectScale<ApplyRectScale>(int_tag, RoundShift<Bit>(int_tag, i3));
    s4 = RectScale<ApplyRectScale>(int_tag, RoundShift<Bit>(int_tag, i4));
    s5 = RectScale<ApplyRectScale>(int_tag, RoundShift<Bit>(int_tag, i5));
    s6 = RectScale<ApplyRectScale>(int_tag, RoundShift<Bit>(int_tag, i6));
    s7 = RectScale<ApplyRectScale>(int_tag, RoundShift<Bit>(int_tag, i7));
  } else {
    auto ip0 = hn::PromoteTo(out_tag, i0);
    auto ip1 = hn::PromoteTo(out_tag, i1);
    auto ip2 = hn::PromoteTo(out_tag, i2);
    auto ip3 = hn::PromoteTo(out_tag, i3);
    auto ip4 = hn::PromoteTo(out_tag, i4);
    auto ip5 = hn::PromoteTo(out_tag, i5);
    auto ip6 = hn::PromoteTo(out_tag, i6);
    auto ip7 = hn::PromoteTo(out_tag, i7);
...
HWY_ATTR HWY_INLINE hn::VFromD<D> LocalInterleaveEvenBlocks(D tag,
  ...
  static_assert(sizeof(hn::TFromD<D>) == 8,
                "Hacked up LocalInterleaveEvenBlocks requires 64-bit lanes.");
...
HWY_ATTR HWY_INLINE hn::VFromD<D> LocalInterleaveOddBlocks(D tag,
                                                           ...
                                                           hn::VFromD<D> b) {
  static_assert(sizeof(hn::TFromD<D>) == 8,
                "Hacked up LocalInterleaveOddBlocks requires 64-bit lanes.");
...
template <int8_t Bit, bool ApplyRectScale, typename D>
HWY_ATTR HWY_INLINE void Transpose16x16x32(D int_tag,
                                           const hn::TFromD<D> *HWY_RESTRICT in,
                                           hn::TFromD<D> *HWY_RESTRICT out,
                                           size_t instride, size_t outstride) {
  static_assert(hn::MaxLanes(int_tag) == 16,
                "Transpose16x16x32 requires an 16-lane int_tag");
  constexpr hn::RepartitionToWide<D> wide_int_tag;
  ...
  for (size_t i = 0; i < 16; i += 4) {
    for (size_t j = 0; j < 2; ++j) {
      auto i0 =
          hn::Load(wide_int_tag, &z[(i + j + 0) * hn::MaxLanes(wide_int_tag)]);
      auto i2 =
          hn::Load(wide_int_tag, &z[(i + j + 2) * hn::MaxLanes(wide_int_tag)]);
  ...
  for (size_t i = 0; i < 16; i += 8) {
    for (size_t j = 0; j < 4; ++j) {
      auto i0 =
          hn::Load(wide_int_tag, &z[(i + j + 0) * hn::MaxLanes(wide_int_tag)]);
      auto i4 =
          hn::Load(wide_int_tag, &z[(i + j + 4) * hn::MaxLanes(wide_int_tag)]);
  ...
  for (size_t j = 0; j < 8; ++j) {
    auto i0 = hn::Load(wide_int_tag, &z[(j + 0) * hn::MaxLanes(wide_int_tag)]);
    auto i8 = hn::Load(wide_int_tag, &z[(j + 8) * hn::MaxLanes(wide_int_tag)]);
    hn::StoreU(hn::BitCast(int_tag, hn::ConcatLowerLower(wide_int_tag, i8, i0)),
               int_tag, &out[kStoreIndex[j + 0] * outstride]);
    hn::StoreU(hn::BitCast(int_tag, hn::ConcatUpperUpper(wide_int_tag, i8, i0)),
               int_tag, &out[kStoreIndex[j + 8] * outstride]);
  }
}

template <int8_t Bit, bool ApplyRectScale, typename D>
HWY_ATTR HWY_INLINE void Transpose16x16x16(D int_tag,
                                           const hn::TFromD<D> *HWY_RESTRICT in,
                                           hn::TFromD<D> *HWY_RESTRICT out,
                                           size_t instride, size_t outstride) {
  static_assert(hn::MaxLanes(int_tag) == 16,
                "Transpose16x16x16 requires an 16-lane int_tag");
  constexpr hn::RepartitionToWide<D> wide_int_tag;
  ...
  for (size_t i = 0; i < 16; i += 4) {
    for (size_t j = 0; j < 2; ++j) {
      auto i0 =
          hn::Load(wide_int_tag, &y[(i + j + 0) * hn::MaxLanes(wide_int_tag)]);
      auto i2 =
          hn::Load(wide_int_tag, &y[(i + j + 2) * hn::MaxLanes(wide_int_tag)]);
...
template <int8_t Bit, bool ApplyRectScale, typename TIn, typename TOut>
HWY_ATTR HWY_INLINE void Transpose16x16(const TIn *HWY_RESTRICT in,
                                        TOut *HWY_RESTRICT out, size_t instride,
                                        size_t outstride) {
  static_assert(sizeof(TOut) == sizeof(TIn),
                "Transpose16x16 does not support integer promotion.");
  constexpr hn::FixedTag<TIn, 16> int_tag;
  if constexpr (sizeof(TIn) == 4) {
    Transpose16x16x32<Bit, ApplyRectScale>(int_tag, in, out, instride,
                                           outstride);
  } else if constexpr (sizeof(TIn) == 2) {
    Transpose16x16x16<Bit, ApplyRectScale>(int_tag, in, out, instride,
                                           outstride);
  } else {
    static_assert(false, "Transpose16x16 requires a 16-bit or 32-bit input.");
  }
}
...
HWY_ATTR HWY_INLINE void Transpose(const TIn *HWY_RESTRICT in,
  ...
  constexpr auto int_tag =
      hn::CappedTag<TOut, std::min<size_t>(16, std::min(Width, Height))>();
  constexpr bool kRequiresPromotion = sizeof(TIn) != sizeof(TOut);
  if constexpr (hn::MaxLanes(int_tag) == 16) {
    if constexpr (kRequiresPromotion) {
      ...
      for (size_t r = 0; r < Height; r += 16) {
        for (size_t c = 0; c < Width; c += 16) {
          ...
          Transpose16x16<Bit, ApplyRectScale>(p, &out[c * outstride + r],
      ...
      for (size_t r = 0; r < Height; r += 16) {
        for (size_t c = 0; c < Width; c += 16) {
          Transpose16x16<Bit, ApplyRectScale>(&in[r * instride + c],
                                              &out[c * outstride + r], instride,
                                              outstride);
        }
      }
    }
  } else if constexpr (hn::MaxLanes(int_tag) >= 8) {
    for (size_t r = 0; r < Height; r += 8) {
      for (size_t c = 0; c < Width; c += 8) {
        Transpose8x8<Bit, ApplyRectScale>(&in[r * instride + c],
                                          &out[c * outstride + r], instride,
                                          outstride);
      }
    }
  } else if constexpr (hn::MaxLanes(int_tag) >= 4) {
    for (size_t r = 0; r < Height; r += 4) {
      for (size_t c = 0; c < Width; c += 4) {
        Transpose4x4<Bit, ApplyRectScale>(&in[r * instride + c],
                                          &out[c * outstride + r], instride,
                                          outstride);
      }
    }
  } else {
    static_assert(false, "Unsupported vector type in Transpose");
  }
}
...
HWY_ATTR HWY_INLINE void StoreBlock(const TIn *HWY_RESTRICT in, size_t instride,
  ...
  constexpr hn::Rebind<TOut, hn::Half<decltype(load_tag)>> store_tag;
  for (size_t r = 0; r < Height; ++r) {
    for (size_t c = 0; c < Width; c += hn::MaxLanes(load_tag)) {
      ...
      if constexpr (std::is_same_v<TIn, TOut>) {
        hn::StoreU(v, load_tag, &out[r * outstride + c]);
      } else {
        hn::StoreU(hn::PromoteLowerTo(store_tag, v), store_tag,
                   &out[r * outstride + c]);
        hn::StoreU(hn::PromoteUpperTo(store_tag, v), store_tag,
                   &out[r * outstride + c + hn::MaxLanes(store_tag)]);
      }
    }
  }
}
...
HWY_ATTR HWY_INLINE void LoadLine(const TInput *HWY_RESTRICT input,
  ...
  for (size_t x = 0; x < Width / hn::MaxLanes(load_tag); ++x) {
    ...
    if constexpr (FlipLeftRight) {
      v = hn::Reverse(load_tag, v);
    }
    if constexpr (std::is_same_v<TInput, TIn>) {
      hn::Store(
          hn::ShiftLeft<Shift>(v), load_tag,
          &in[(FlipLeftRight ? (Width / hn::MaxLanes(load_tag)) - x - 1 : x) *
              hn::MaxLanes(load_tag)]);
    } else {
      hn::Store(
          hn::ShiftLeft<Shift>(hn::PromoteTo(store_tag, v)), store_tag,
...
HWY_ATTR HWY_INLINE HWY_MAYBE_UNUSED void ForwardTransform2D(
    const int16_t *input, int32_t *output, size_t stride, TX_TYPE tx_type) {
  ...
  constexpr size_t kBlockWidth =
      std::max<size_t>(kMinVectorSize / sizeof(T), kWidth);
  constexpr size_t kBlockHeight =
      std::max<size_t>(kMinVectorSize / sizeof(T), kHeight);
  ...
  using T2 = std::conditional_t<kBigRectangle, int32_t, T>;
  ...
  constexpr int kRectType = RectangularTransformLogRatio<kWidth, kHeight>();
  constexpr bool kApplyRectScale = std::max(kRectType, -kRectType) % 2 == 1;
  ...
  if constexpr (kHeight <= 16 && kWidth == 64) {
