...
static void set_allintra_speed_features_framesize_independent(
  ...
  if (speed >= 1) {
    ...
    sf->mv_sf.prune_intrabc_candidate_block_hash = 1;
  ...
  if (speed >= 6) {
    ...
    sf->mv_sf.top_only_intrabc_search = 1;
...
static inline void init_mv_sf(MV_SPEED_FEATURES *mv_sf) {
  ...
  mv_sf->use_intrabc = 1;
  mv_sf->prune_intrabc_candidate_block_hash = 0;
  mv_sf->top_only_intrabc_search = 0;
