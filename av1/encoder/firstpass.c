...
static int firstpass_inter_prediction(
  ...
  if ((current_frame->frame_number > 2) && last2_frame != NULL) {
    ...
    /*
    if (last2_motion_error < motion_error) {
      motion_error = last2_motion_error;
      mv = tmp_mv;
    }
    */
...
void av1_first_pass(AV1_COMP *cpi, const int64_t ts_duration) {
  ...
  // fprintf(stderr, "frame id = %d, coded error = %lf, lt error = %lf\n",
  //         current_frame->frame_number, this_frame_stats->coded_error,
  //         this_frame_stats->lt_coded_error);

  if (this_frame_stats->pcnt_inter < 0.2)
