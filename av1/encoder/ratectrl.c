...
static int fast_detection_scroll(const AV1_COMP *cpi,
                                 const EncodeFrameInput *frame_input,
                                 unsigned int y_sad_thr, BLOCK_SIZE bsize) {
  ...
  for (int pts = 0; pts < 3; pts++) {
    ...
    const int offset = bsize == BLOCK_128X128 ? 64 : 32;
    const int pos_col = (fac * unscaled_src->y_width >> shift) - offset;
    const int pos_row = (fac * unscaled_src->y_height >> shift) - offset;
    if (pos_col >= 0 && pos_col < unscaled_src->y_width - offset &&
        pos_row >= 0 && pos_row < unscaled_src->y_height - offset) {
      int src_ystride = unscaled_src->y_stride;
      int last_src_ystride = unscaled_last_src->y_stride;
      uint8_t *src_y = unscaled_src->y_buffer + pos_row * src_ystride + pos_col;
      uint8_t *last_src_y =
          unscaled_last_src->y_buffer + pos_row * last_src_ystride + pos_col;
      int best_intmv_col = 0;
      int best_intmv_row = 0;
      unsigned int sse;
      const unsigned int spatial_var =
          cpi->ppi->fn_ptr[bsize].vf(src_y, src_ystride, AV1_VAR_OFFS, 0, &sse);
      if (spatial_var > 1000) {
        unsigned int y_sad = estimate_scroll_motion(
            cpi, src_y, last_src_y, src_ystride, last_src_ystride, bsize,
...
static void rc_scene_detection_onepass_rt(AV1_COMP *cpi,
  ...
  if (rc->high_source_sad) {
    ...
    if (cpi->oxcf.tune_cfg.content == AOM_CONTENT_SCREEN &&
      ...
      scroll_detected_frame_level =
          fast_detection_scroll(cpi, frame_input, 1, BLOCK_128X128);
      scroll_tested = 1;
      if (scroll_detected_frame_level) rc->high_source_sad = 0;
    }
  }
  ...
  if (cpi->svc.temporal_layer_id == 0 && rc->drop_count_consec < 3) {
    ...
    if (cpi->oxcf.speed >= 11 &&
      ...
      if (scroll_tested && scroll_detected_frame_level) {
        ...
        if (fast_detection_scroll(cpi, frame_input, 100, BLOCK_128X128))
          cpi->rc.high_motion_content_screen_rtc = 0;
      }
    }
    ...
    *q = (cpi->rc.worst_quality + *q) >> 1;
    ...
    if (cpi->sf.rt_sf.rc_compute_spatial_var_sc) {
      if (cpi->rc.frame_spatial_variance < 100 ||
          (cpi->rc.percent_blocks_with_motion < 30 &&
           cpi->rc.frame_source_sad < 120000)) {
        *q = (cpi->rc.worst_quality + *q) >> 1;
