...
void av1_free_tpl_gop_stats(AomTplGopStats *extrc_tpl_gop_stats) {
  ...
  for (int frame_index = 0; frame_index < extrc_tpl_gop_stats->size;
       ++frame_index) {
    aom_free(
        extrc_tpl_gop_stats->frame_stats_list[frame_index].block_stats_list);
    extrc_tpl_gop_stats->frame_stats_list[frame_index].block_stats_list = NULL;
  }
  ...
  for (int frame_index = 0; frame_index < tpl_gop_frames; ++frame_index) {
    ...
    AOM_CHECK_MEM_ERROR(
        ...
        aom_calloc(mi_rows * mi_cols,
                   sizeof(*extrc_tpl_gop_stats->frame_stats_list[frame_index]
                               .block_stats_list)));
    extrc_tpl_gop_stats->frame_stats_list[frame_index].num_blocks =
        mi_rows * mi_cols;
    extrc_tpl_gop_stats->frame_stats_list[frame_index].frame_width =
        frame_width;
    extrc_tpl_gop_stats->frame_stats_list[frame_index].frame_height =
        frame_height;
  }
}
