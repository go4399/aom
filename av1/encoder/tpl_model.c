...
void av1_setup_tpl_buffers(AV1_PRIMARY *const ppi,
  ...
  tpl_data->border_in_pixels = ppi->cpi->oxcf.border_in_pixels;
...
static uint32_t motion_estimation(AV1_COMP *cpi, MACROBLOCK *x,
  ...
  if (search_site_cfg->stride != ref_stride) {
    fprintf(stderr, "cfg stride = %d, cfg stride = %d, ref stride = %d\n",
            cpi->mv_search_params.search_site_cfg[SS_CFG_SRC]->stride,
            cpi->mv_search_params.search_site_cfg[SS_CFG_FPF]->stride,
            ref_stride);
  }
