...
int_mv av1_simple_motion_search_ext(AV1_COMP *const cpi,
  ...
  av1_set_offsets(cpi, tile, x, mi_row, mi_col, bsize);
  // set_offsets_for_motion_search(cpi, x, mi_row, mi_col, bsize);
  ...
  // ref_mv is used to calculate the cost of the motion vector
  const MV ref_mv = kZeroMv;
#if 0 // CONFIG_MV_RANGE_EXTENSION
  ...
  if (use_subpel_search) {
    ...
    cpi->mv_search_params.find_fractional_mv_step(
        xd, cm, &ms_params, subpel_start_mv, &best_mv_stats, &best_mv.as_mv, &not_used,
        &x->pred_sse[ref], NULL);
  } else {
    // Manually convert from units of pixel to 1/8-pixels if we are not doing
    // subpel search
