...
static inline void update_coeff_general(
  ...
  if (qc == 0) {
    *accu_rate += txb_costs->base_cost[coeff_ctx][0];
    ...
    const int rate_diff = rate - rate_low;
    const int64_t dist_diff = (dist_diff_low_0 - dist_diff_0) * 1024;
    if ((int64_t)rate_diff * rdmult > dist_diff) {
      qcoeff[ci] = qc_low;
      dqcoeff[ci] = dqc_low;
    ...
    *accu_rate += txb_costs->base_cost[coeff_ctx][0];
    ...
    if (abs_qc == 1) {
      ...
      if (allow_lower_qc) {
        int64_t dist_diff;
        if (qmatrix == NULL) {
          const int64_t dist_diff_base =
              (int64_t)abs_dqc * ((abs_tqc << 1) - abs_dqc);
          dist_diff = dist_diff_base * (1 << (2 * shift + 10));
        } else {
          const int64_t dist =
              get_coeff_dist(abs_tqc, abs_dqc, shift, qmatrix, ci);
          const int64_t dist_low =
              get_coeff_dist(abs_tqc, 0, shift, qmatrix, ci);
          dist_diff = (dist_low - dist) * 1024;
        }
        const int rate_low = rate - base_cost[5];
        if ((int64_t)base_cost[5] * rdmult > dist_diff) {
          qcoeff[ci] = 0;
          dqcoeff[ci] = 0;
      ...
      *accu_rate += rate;
      ...
      if (allow_lower_qc) {
        ...
        if (qmatrix == NULL) {
          const int64_t dist_diff_base =
              (int64_t)(abs_dqc - abs_dqc_low) *
              ((abs_tqc << 1) - (abs_dqc + abs_dqc_low));
          dist_diff = dist_diff_base * (1 << (2 * shift + 10));
        } else {
          const int64_t dist =
              get_coeff_dist(abs_tqc, abs_dqc, shift, qmatrix, ci);
