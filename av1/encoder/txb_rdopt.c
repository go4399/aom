  ...
  if (qc == 0) {
    *accu_rate += txb_costs->base_cost[coeff_ctx][0];
    ...
    if (qmatrix == NULL) {
      ...
      dist_diff_0 = ((int64_t)dqc * (dqc - tqc2)) << (2 * shift);
      dist_diff_low_0 = (abs_qc == 1) ? 0
                                      : (((int64_t)dqc_low * (dqc_low - tqc2))
                                         << (2 * shift));
    } else {
    ...
    *accu_rate += txb_costs->base_cost[coeff_ctx][0];
    ...
    if (abs_qc == 1) {
      ...
      if (allow_lower_qc) {
        int64_t dist_diff_0;
        if (qmatrix == NULL) {
          dist_diff_0 = ((int64_t)abs_dqc * (abs_dqc - ((int64_t)abs_tqc * 2)))
                        << (2 * shift);
        } else {
      ...
      *accu_rate += rate;
      ...
      if (allow_lower_qc) {
        ...
        if (qmatrix == NULL) {
          ...
          dist_diff_0 = ((int64_t)abs_dqc * (abs_dqc - abs_tqc2))
                        << (2 * shift);
...
static AOM_FORCE_INLINE void update_coeff_eob(
  ...
  if (qc == 0) {
    *accu_rate += txb_costs->base_cost[coeff_ctx][0];
    ...
    if (qmatrix == NULL) {
      const int64_t tqc2 = (int64_t)tqc * 2;
      dist = ((int64_t)dqc * (dqc - tqc2)) << (2 * shift);
      dist_low = (abs_qc == 1)
                     ? 0
                     : (((int64_t)dqc_low * (dqc_low - tqc2)) << (2 * shift));
    } else {
      const int64_t dist0 = get_coeff_dist(tqc, 0, shift, qmatrix, ci);
