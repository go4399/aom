...
static void tf_motion_search(AV1_COMP *cpi, MACROBLOCK *mb,
  ...
  *is_low_cntras = 0;
  ...
  int64_t src_var = cpi->ppi->fn_ptr[block_size].vf(
      mb->plane[0].src.buf, y_stride, dclevel, dclevel_stride, &sse);
  ...
  if (force_integer_mv == 1) {  // Only do full search on the entire block.
    ...
    if (src_var <= 2 * error) *is_low_cntras = 1;
    ...
    *is_dc_diff_large = 50 * error < sse;
    if (src_var <= 2 * distortion) *is_low_cntras = 1;
...
void av1_tf_do_filtering_row(AV1_COMP *cpi, ThreadData *td, int mb_row) {
  ...
  for (int mb_col = 0; mb_col < tf_ctx->mb_cols; mb_col++) {
    ...
    for (int frame = 0; frame < num_frames; frame++) {
      ...
      if (is_low_cntras) filter_strength = AOMMIN(filter_strength, 1);
