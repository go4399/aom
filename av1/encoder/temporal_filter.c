...
static void tf_build_predictor(const YV12_BUFFER_CONFIG *const frame_to_filter,
  ...
  DECLARE_ALIGNED(32, int16_t, src_diff[16 * 16]);
  DECLARE_ALIGNED(32, tran_low_t, coeff[16 * 16]);
  DECLARE_ALIGNED(32, tran_low_t, qcoeff[16 * 16]);
  DECLARE_ALIGNED(32, tran_low_t, dqcoeff[16 * 16]);
    ...
    for (int i = 0; i < plane_h; i += h) {
      for (int j = 0; j < plane_w; j += w) {
        ...
        if (plane == 0) {
          ...
          if (eob) subblock_mses[subblock_idx] = INT_MAX;
        }
  ...
  for (int plane = 0; plane < num_planes; ++plane) {
    ...
    for (int i = 0; i < h; ++i) {
      for (int j = 0; j < w; ++j) {
        ...
        if (subblock_mses[subblock_idx] == INT_MAX) {
          ++pred_idx;
          continue;
        }
