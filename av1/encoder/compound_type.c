...
// mask.
static void do_comp_wedge_search(
    const AV1_COMP *const cpi, MACROBLOCK *x, const int_mv *const cur_mv,
    ...
    const PREDICTION_MODE this_mode, int rate_mv, int64_t ref_best_rd,
    int64_t ref_skip_rd, int masked_type_cost, int64_t *best_rd_cur,
    int *tmp_rate_mv, int *rs2) {
  const AV1_COMMON *cm = &cpi->common;
  MACROBLOCKD *xd = &x->e_mbd;
...
// mask.
static void do_comp_wedge_search_using_modelrd(
    const AV1_COMP *const cpi, MACROBLOCK *x, const int_mv *const cur_mv,
    const BUFFER_SET *orig_dst, const CompoundTypeRdBuffers *buffers,
    uint8_t **preds0, uint8_t **preds1, int *strides, const BLOCK_SIZE bsize,
    const PREDICTION_MODE this_mode, int rate_mv, int64_t ref_skip_rd,
    int masked_type_cost, int *calc_pred_masked_compound, int64_t *best_rd_cur,
    int *tmp_rate_mv, int *rs2) {
  const AV1_COMMON *cm = &cpi->common;
  MACROBLOCKD *xd = &x->e_mbd;
  MB_MODE_INFO *mbmi = xd->mi[0];
  ...
  *rd = INT64_MAX;
  ...
  for (int i = 0; i < valid_type_count; i++) {
    ...
    if (cur_type < COMPOUND_WEDGE) {
      ...
      if (!cpi->sf.inter_sf.enable_fast_comp_wedge_eval) {
        do_comp_wedge_search(
            cpi, x, cur_mv, orig_dst, args, rd_stats, preds0, preds1, strides,
            bsize, this_mode, *rate_mv, ref_best_rd, ref_skip_rd,
            masked_type_cost[cur_type], &best_rd_cur, &tmp_rate_mv, &rs2);
      } else {
        do_comp_wedge_search_using_modelrd(
            cpi, x, cur_mv, orig_dst, buffers, preds0, preds1, strides, bsize,
            this_mode, *rate_mv, ref_skip_rd, masked_type_cost[cur_type],
            &calc_pred_masked_compound, &best_rd_cur, &tmp_rate_mv, &rs2);
      }
