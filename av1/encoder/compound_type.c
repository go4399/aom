  ...
  *rd = INT64_MAX;
  ...
  for (int i = 0; i < valid_type_count; i++) {
    ...
    if (cur_type < COMPOUND_WEDGE) {
      ...
      do_comp_wedge_search(cpi, x, cur_mv, orig_dst, args, rd_stats, preds0,
                           preds1, strides, bsize, this_mode, *rate_mv,
                           ref_best_rd, ref_skip_rd, masked_type_cost[cur_type],
