  ...
  if (cpi->oxcf.q_cfg.use_fixed_qp_offsets == 2 &&
    ...
    *q = av1_rc_pick_q_and_bounds(cpi, cm->width, cm->height,
    ...
    if (cpi->oxcf.q_cfg.use_fixed_qp_offsets &&
        cpi->oxcf.rc_cfg.mode == AOM_Q) {
      // cq_level is converted to [0, 323] range in by
      // av1_quantizer_to_qindex(extra_cfg->cq_level) function
      const int cq_level = cpi->oxcf.rc_cfg.cq_level;
      printf("%s:%d cq_level = %d \n", __FILE__, __LINE__, cq_level)
      ...
      if (is_frame_tpl_eligible(gf_group, cpi->gf_frame_index)) {
        ...
        if (cq_level <= MAXQ) {
          ...
          // so all layers converge to MAXQ at MAX_CQ_LEVEL_QINDEX (cq=80).
          qstep_ratio =
              0.2 +
              (double)(cq_level - MAXQ) / (MAX_CQ_LEVEL_QINDEX - MAXQ) * 0.8;
        }
        if (qstep_ratio >= 1.0)
