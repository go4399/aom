...
void av1_cdef_search(AV1_COMP *cpi) {
  ...
  // If running adaptive CDEF with strength zeroing, let search derive at
  // least two (luma, chroma) strength pairs. Doing so will find low strengths
  // that can be zeroed out to reduce overall decode time.
  ...
  if (should_reduce_cdef_strengths) {
    for (int j = 0; j < cdef_info->nb_cdef_strengths; j++) {
      ...
      // Zero out entries with low luma and chroma strengths. This is done to
      // reduce decode time, as CDEF is a relatively-expensive filter to
      // compute. The low-strength thresholds were empirically derived from
      // subjective testing and SSIMULACRA 2 scores. These strike a balance
      // between perceptual quality gains and a reasonable single-threaded
      // decode time increase (~10%) over --enable-cdef 0.
      // There's an overall 0.18 point loss in SSIMULACRA 2 scores over no CDEF
      // strength zeroing at speed 6, QP 30 on the CLIC 2020 dataset.
