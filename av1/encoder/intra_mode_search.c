...
static int rd_pick_filter_intra_sby(
    ...
    int64_t *distortion, int64_t *sse, uint8_t *skippable, BLOCK_SIZE bsize,
    int mode_cost, PREDICTION_MODE best_mode_so_far, int64_t *best_rd,
  ...
  for (mode = 0; mode < FILTER_INTRA_MODES; ++mode) {
    ...
    const int tx_select =
        x->txfm_search_params.tx_mode_search_type == TX_MODE_SELECT &&
        block_signals_txsize(mbmi->bsize);
    int tx_size_rate = 0;
    if (tx_select) {
      const int txfm_partition_ctx =
          txfm_partition_context(xd->above_txfm_context, xd->left_txfm_context,
                                 mbmi->bsize, mbmi->tx_size);
      tx_size_rate =
          is_inter_block(mbmi)
              ? x->mode_costs.txfm_partition_cost[txfm_partition_ctx][0]
              : tx_size_cost(x, bsize, mbmi->tx_size);
    }

    this_rd = AOMMIN(
        this_rd,
        RDCOST(x->rdmult,
               x->mode_costs.skip_txfm_cost[skip_ctx][1] + tx_size_rate +
                   intra_mode_info_cost_y(cpi, x, mbmi, bsize, mode_cost, 0),
               tokenonly_rd_stats.sse));
...
int64_t av1_rd_pick_intra_sbuv_mode(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  for (int mode_idx = 0; mode_idx < UV_INTRA_MODES; ++mode_idx) {
    ...
    int skip_rate = x->mode_costs.skip_txfm_cost[skip_ctx][1] + tx_size_rate +
                    +intra_mode_info_cost_uv(cpi, x, mbmi, bsize, mode_cost);
...
int av1_search_palette_mode(IntraModeSearchState *intra_search_state,
  ...
  if (num_planes > 1) {
    if (intra_search_state->rate_uv_intra == INT_MAX) {
      ...
      av1_rd_pick_intra_sbuv_mode(cpi, x, &intra_search_state->rate_uv_intra,
                                  ...
                                  &intra_search_state->dist_uvs, &sse2,
  ...
  if (is_inter_block(mbmi)) rate2 += tx_size_rate;
...
void av1_search_palette_mode_luma(const AV1_COMP *cpi, MACROBLOCK *x,
  ...
  if (is_inter_block(mbmi)) rd_stats_y.rate += tx_size_rate;
...
static inline int intra_block_yrd(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  int64_t this_rd = RDCOST(x->rdmult, this_rate, rd_stats.dist);
  const int skip_ctx = av1_get_skip_txfm_context(xd);

  this_rd = AOMMIN(
      this_rd, RDCOST(x->rdmult,
                      x->mode_costs.skip_txfm_cost[skip_ctx][1] +
                          tx_size_cost(x, bsize, mbmi->tx_size) +
                          intra_mode_info_cost_y(cpi, x, mbmi, bsize,
                                                 bmode_costs[mbmi->mode], 0),
                      rd_stats.sse));
...
int64_t av1_rd_pick_intra_sby_mode(const AV1_COMP *const cpi, MACROBLOCK *x,
  ...
  for (int mode_idx = INTRA_MODE_START; mode_idx < LUMA_MODE_COUNT;
    ...
    this_rd = AOMMIN(
        this_rd,
        RDCOST(x->rdmult,
               skip_txfm_cost[1] + tx_size_cost(x, bsize, mbmi->tx_size) +
                   intra_mode_info_cost_y(cpi, x, mbmi, bsize,
                                          bmode_costs[mbmi->mode], 0),
               this_sse));
  ...
  if (beat_best_rd && av1_filter_intra_allowed_bsize(&cpi->common, bsize)) {
    if (rd_pick_filter_intra_sby(cpi, x, rate, rate_tokenonly, distortion, sse,
      ...
      *mbmi = x->winner_mode_stats[mode_idx].mbmi;
      if (is_winner_mode_processing_enabled(cpi, x, mbmi, 0)) {
        ...
        if (intra_block_yrd(cpi, x, bsize, bmode_costs, &best_rd, rate,
                            rate_tokenonly, distortion, sse, skippable,
                            &best_mbmi, ctx))
    ...
    if (is_winner_mode_processing_enabled(cpi, x, mbmi, 0)) {
      ...
      *mbmi = best_mbmi;
      intra_block_yrd(cpi, x, bsize, bmode_costs, &best_rd, rate,
                      rate_tokenonly, distortion, sse, skippable, &best_mbmi,
                      ctx);
    }
  }
  *mbmi = best_mbmi;
