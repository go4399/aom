...
int av1_compute_rd_mult_based_on_qindex(aom_bit_depth_t bit_depth,
  ...
  if (tuning == AOM_TUNE_IQ || tuning == AOM_TUNE_SSIMULACRA2) {
    ...
    // This weight part of the equation was determined by iteratively
    // increasing weight on CID22 and Daala's subset1, and observing its
    // effects on visual quality and SSIMULACRA 2 scores along the usable
    // (0-100) range.
    if (mode == REALTIME) {
      ...
      // stronger bias towards either choosing intra prediction modes, or for
      // inter prediction modes to preserve coefficient residuals (i.e. no
      // skip mode).
      weight = 32;
    } else {
      // All-intra and good-quality modes: Further multiply rdmult
      // (by up to 200/128 = 1.5625) to improve image quality. The most
      // noticeable effect is a mild bias towards choosing larger transform
      // sizes (e.g. one 16x16 transform instead of 4 8x8 transforms).
