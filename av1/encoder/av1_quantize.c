...
void av1_set_quantizer(AV1_COMMON *const cm, int min_qmlevel, int max_qmlevel,
  ...
  if (enable_chroma_deltaq) {
    if (is_allintra && tuning == AOM_TUNE_PERCEPTUAL) {
  ...
  if (is_allintra) {
    if (tuning == AOM_TUNE_PERCEPTUAL) {
      // Use luma QM formula specifically tailored for tune perceptual
      get_luma_qmlevel = aom_get_qmlevel_luma_perceptual;
      ...
      if (cm->seq_params->subsampling_x == 0 &&
        ...
        get_chroma_qmlevel = aom_get_qmlevel_444_chroma_perceptual;
      } else {
