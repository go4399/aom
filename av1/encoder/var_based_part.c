...
int av1_choose_var_based_partitioning(AV1_COMP *cpi, const TileInfo *const tile,
  ...
  // Allow early exit with sb partition size for spatially flat areas.
  // Only for screen content on slide changes for now.
  const int block_width = mi_size_wide[cm->seq_params->sb_size];
  const int block_height = mi_size_high[cm->seq_params->sb_size];
  if (!is_key_frame && cpi->oxcf.tune_cfg.content == AOM_CONTENT_SCREEN && x->source_variance == 0 && cpi->rc.high_source_sad &&
