...
static inline void encode_frame_internal(AV1_COMP *cpi) {
  ...
  if (!is_stat_generation_stage(cpi) && av1_use_hash_me(cpi) &&
    ...
    size_t block_hash_size =
        sizeof(*block_hash_values[0]) * pic_width * pic_height;
    bool error = false;

    for (int j = 0; j < 2; ++j) {
      block_hash_values[j] = (uint32_t *)aom_malloc(block_hash_size);
    ...
    for (int size = 4; size <= max_sb_size; size *= 2, src_idx = !src_idx) {
      ...
      av1_generate_block_hash_value(
          ...
          block_hash_values[dst_idx], block_hash_size);
