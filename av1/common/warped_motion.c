...
// This is an exact duplicate of the table above but with the recognition
// that all the values are 8-bit, so the type is int8_t. Duplicating the
// table is a temporary workaround until the intrinsic code paths can
// coalesce to the change. The x86 path already has an 8-bit version of the
// table (see av1_filter_8bit in av1/common/x86/warp_plane_sse4.c).
// Eventually, the table below (av1_warped_filter_8bit) will go away and
// the above table (av1_warped_filter) will be typed int8_t. This enables
// 8-bit computation instructions and reduces memory bandwidth.
//
    ...
    for (int j = p_col; j < p_col + p_width; j += 8) {
      ...
      for (int k = -7; k < 8; ++k) {
        ...
        for (int l = -4; l < 4; ++l) {
          ...
          const int8_t *coeffs = av1_warped_filter_8bit[offs];
