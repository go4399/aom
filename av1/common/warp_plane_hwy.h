...
HWY_ATTR inline void WarpAffineTemplate(
  ...
  const int64_t dst_x_stride_j = (int64_t)mat[2] * (8 << subsampling_x);
  const int64_t dst_y_stride_j = (int64_t)mat[4] * (8 << subsampling_x);
  const int64_t dst_x_stride_i = (int64_t)mat[3] * (8 << subsampling_y);
  const int64_t dst_y_stride_i = (int64_t)mat[5] * (8 << subsampling_y);

  const int32_t src_x_0 = (p_col + 4) << subsampling_x;
  const int32_t src_y_0 = (p_row + 4) << subsampling_y;
  const int64_t dst_x_start =
      (int64_t)mat[2] * src_x_0 + (int64_t)mat[3] * src_y_0 + (int64_t)mat[0];
  const int64_t dst_y_start =
      (int64_t)mat[4] * src_x_0 + (int64_t)mat[5] * src_y_0 + (int64_t)mat[1];
