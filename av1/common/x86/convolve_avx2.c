...
static inline void av1_convolve_x_sr_general_avx2(
  ...
  if (horiz_tap == 4) {
    ...
    if (w == 8) {
      ...
      do {
        load_convolve_round_8tap_32_avx2(src_ptr, coeffs, filt, dst);
        load_convolve_round_8tap_32_avx2(src_ptr + 1 * 32, coeffs, filt,
                                         dst + 1 * 32);
        load_convolve_round_8tap_32_avx2(src_ptr + 2 * 32, coeffs, filt,
                                         dst + 2 * 32);
        load_convolve_round_8tap_32_avx2(src_ptr + 3 * 32, coeffs, filt,
                                         dst + 3 * 32);
        src_ptr += src_stride;
        dst += dst_stride;
      } while (--h);
