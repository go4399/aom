...
static aom_codec_err_t validate_config(aom_codec_alg_priv_t *ctx,
  ...
  RANGE_CHECK(extra_cfg, tuning, AOM_TUNE_PSNR, AOM_TUNE_PERCEPTUAL);
...
static aom_codec_err_t handle_tuning(aom_codec_alg_priv_t *ctx,
                                     struct av1_extracfg *extra_cfg) {
  if (extra_cfg->tuning == AOM_TUNE_PERCEPTUAL) {
    ...
    // - aom_get_qmlevel_luma_perceptual()
    // - aom_get_qmlevel_444_chroma_perceptual()
    extra_cfg->enable_qm = 1;
    extra_cfg->qm_min = QM_FIRST_PERCEPTUAL;
    extra_cfg->qm_max = QM_LAST_PERCEPTUAL;
