...
static aom_codec_err_t ctrl_set_svc_params(aom_codec_alg_priv_t *ctx,
  ...
  if (cpi->svc.prev_number_temporal_layers &&
    ...
    // Check for valid values for the spatial/temporal_layer_id here, since there
    // has been a dynamic change in the number_spatial/temporal_layers, and if the
    // control SET_SVC_LAYER_ID is not used after this call, the previous (last_encoded)
    // values of spatial/temporal_layer_id will be used, which may be invalid.
    cpi->svc.spatial_layer_id =
        AOMMAX(0, AOMMIN(cpi->svc.spatial_layer_id, cpi->svc.number_spatial_layers - 1));
    cpi->svc.temporal_layer_id =
        AOMMAX(0, AOMMIN(cpi->svc.temporal_layer_id, cpi->svc.number_temporal_layers - 1));
    cpi->common.spatial_layer_id =
        AOMMAX(0, AOMMIN(cpi->common.spatial_layer_id, cpi->svc.number_spatial_layers - 1));
    cpi->common.temporal_layer_id =
        AOMMAX(0, AOMMIN(cpi->common.temporal_layer_id, cpi->svc.number_temporal_layers - 1));
  }
