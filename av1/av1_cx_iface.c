  ...
  if (res == AOM_CODEC_OK) {
    ...
    if (img != NULL) {
      ...
      if (!ppi->lookahead) {
        ...
        AV1EncoderConfig *oxcf = &cpi->oxcf;
        const int width = ppi->number_spatial_layers > 1 ? cpi->data_alloc_width : oxcf->frm_dim_cfg.width;
        const int height = ppi->number_spatial_layers > 1 ? cpi->data_alloc_height : oxcf->frm_dim_cfg.height;
        const BLOCK_SIZE sb_size = av1_select_sb_size(
            oxcf, width, height, ppi->number_spatial_layers);
        oxcf->border_in_pixels =
            av1_get_enc_border_size(av1_is_resize_needed(oxcf),
