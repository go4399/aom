...
static aom_codec_err_t encoder_encode(aom_codec_alg_priv_t *ctx,
  ...
  if (img != NULL) {
    ...
    if (res == AOM_CODEC_OK) {
      ...
      // frame size. https://g-issues.oss-fuzz.com/issues/514006304 further
      // shows that multithreaded bitstream packing may need more than 2 times
      // the uncompressed frame size. Hence the size of the buffer is chosen as
      // 2.5 times the uncompressed frame size.
