...
static void read_metadata_itut_t35(AV1Decoder *const pbi, const uint8_t *data,
                                   size_t sz, int is_layer_specific) {
  if (sz == 0) {
    aom_internal_error(&pbi->error, AOM_CODEC_CORRUPT_FRAME,
  ...
  alloc_read_metadata(
      pbi, OBU_METADATA_TYPE_ITUT_T35, data, end_index,
      is_layer_specific ? AOM_MIF_ANY_FRAME_LAYER_SPECIFIC : AOM_MIF_ANY_FRAME);
}
  ...
  while (!frame_decoding_finished && pbi->error.error_code == AOM_CODEC_OK) {
    ...
    switch (obu_header.type) {
      ...
      case OBU_TILE_GROUP:
        ...
        break;
      case OBU_METADATA: {
        const int is_layer_specific = obu_header.has_extension;
        decoded_payload_size =
            read_metadata(pbi, data, payload_size, is_layer_specific);
        if (pbi->error.error_code != AOM_CODEC_OK) return -1;
        break;
      }
      case OBU_TILE_LIST:
