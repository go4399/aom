...
function(add_c_flag_if_supported c_flag)
  ...
  if(${C_FLAG_SUPPORTED})
    ...
    endforeach()
    set(AOM_C_FLAGS "${AOM_C_FLAGS}" PARENT_SCOPE)
  else()
    append_flag(AOM_FAILED_C_FLAGS "${c_flag}")
    set(AOM_FAILED_C_FLAGS "${AOM_FAILED_C_FLAGS}" PARENT_SCOPE)
  endif()
endfunction()
...
function(add_cxx_flag_if_supported cxx_flag)
  ...
  if(${CXX_FLAG_SUPPORTED})
    ...
    foreach(config ${AOM_CXX_CONFIGS})
      ...
      set(${config} "${${config}}" PARENT_SCOPE)
    endforeach()
    set(AOM_CXX_FLAGS "${AOM_CXX_FLAGS}" PARENT_SCOPE)
  else()
    append_flag(AOM_FAILED_CXX_FLAGS "${cxx_flag}")
    set(AOM_FAILED_CXX_FLAGS "${AOM_FAILED_CXX_FLAGS}" PARENT_SCOPE)
  endif()
endfunction()
...
function(require_c_flag c_flag update_c_flags)
  ...
  if(update_c_flags)
    foreach(config ${AOM_C_CONFIGS})
      set(new_val "${${config}} ${c_flag}")
      message("DEBUG: require_c_flag: config=${config}, before=${${config}}, appending=${c_flag}")
      set(${config} "${new_val}" CACHE STRING "" FORCE)
      set(${config} "${new_val}" PARENT_SCOPE)
      message("DEBUG: require_c_flag: config=${config}, after=${new_val}")
    endforeach()
  endif()
endfunction()
...
function(require_cxx_flag cxx_flag update_cxx_flags)
  ...
  if(update_cxx_flags)
    foreach(config ${AOM_CXX_CONFIGS})
      set(new_val "${${config}} ${cxx_flag}")
      message("DEBUG: require_cxx_flag: config=${config}, before=${${config}}, appending=${cxx_flag}")
      set(${config} "${new_val}" CACHE STRING "" FORCE)
      set(${config} "${new_val}" PARENT_SCOPE)
      message("DEBUG: require_cxx_flag: config=${config}, after=${new_val}")
    endforeach()
  endif()
endfunction()
