...
// aasert is triggered in intra_mode_search.c, function av1_count_colors_highbd.
TEST(EncodeAPI, Issue471095598) {
  ...
  ASSERT_EQ(aom_codec_enc_init(&encoder, encoder_iface, &cfg,
            ...
            AOM_CODEC_OK);
  ASSERT_EQ(aom_codec_control(&encoder, AOME_SET_CPUUSED, 2), AOM_CODEC_OK);
  ...
  ASSERT_EQ(aom_codec_encode(&encoder, nullptr, 0, 1, 0), AOM_CODEC_OK);
  aom_img_free(img);
