...
static void EncodeOne(aom_codec_ctx_t *codec, aom_image_t *img, int pts) {
...
TEST(EncodeAPI, DynamicSvcAq0Issue499606109) {
  ...
  for (int f = 0; f < 20; ++f) {
    ...
    ASSERT_EQ(aom_codec_control(&codec, AV1E_SET_SVC_LAYER_ID, &lid),
              AOM_CODEC_OK);
    FillImage(raw, (uint8_t)((f % 2 == 0) ? 16 : 235));
    EncodeOne(&codec, raw, pts++);
  }
