...
static void fill_image(aom_image_t *img, uint8_t val) {
  for (int p = 0; p < 3; ++p) {
...
static void set_svc(aom_codec_ctx_t *codec, int num_sl, const int *num,
...
static void encode_one(aom_codec_ctx_t *codec, aom_image_t *img, int pts) {
  ASSERT_EQ(aom_codec_encode(codec, img, (aom_codec_pts_t)pts, 1, 0),
            AOM_CODEC_OK);
...
TEST(EncodeAPI, DynamicSvcIssue499606109) {
  aom_codec_iface_t *iface = aom_codec_av1_cx();
  aom_codec_enc_cfg_t cfg;
  ASSERT_EQ(aom_codec_enc_config_default(iface, &cfg, AOM_USAGE_REALTIME),
            AOM_CODEC_OK);
  ...
  {
    int num[] = { 1, 1 }, den[] = { 1, 1 }, br[] = { 200, 300 };
    set_svc(&codec, 2, num, den, br);
  }
  int pts = 0;
  aom_image_t *raw = aom_img_alloc(NULL, AOM_IMG_FMT_I420, 640, 360, 1);
  for (int f = 0; f < 3; ++f) {
    for (int sl = 0; sl < 2; ++sl) {
      aom_svc_layer_id_t lid = { sl, 0 };
      ASSERT_EQ(aom_codec_control(&codec, AV1E_SET_SVC_LAYER_ID, &lid),
                AOM_CODEC_OK);
      fill_image(raw, (uint8_t)(128 + f));
      encode_one(&codec, raw, pts++);
    }
  }
  aom_img_free(raw);
  ...
  ASSERT_EQ(aom_codec_enc_config_set(&codec, &cfg), AOM_CODEC_OK);
  {
    int num[] = { 1, 2, 1 }, den[] = { 3, 3, 1 }, br[] = { 200, 500, 1300 };
    set_svc(&codec, 3, num, den, br);
  }
  raw = aom_img_alloc(NULL, AOM_IMG_FMT_I420, 1920, 1080, 1);
  for (int f = 0; f < 3; ++f) {
    for (int sl = 0; sl < 3; ++sl) {
      aom_svc_layer_id_t lid = { sl, 0 };
      ASSERT_EQ(aom_codec_control(&codec, AV1E_SET_SVC_LAYER_ID, &lid),
                AOM_CODEC_OK);
      fill_image(raw, (uint8_t)(64 + f));
      encode_one(&codec, raw, pts++);
    }
  }
  aom_img_free(raw);

  // Phase 2: back to 640x360, 2 SLs. Encode SL0 only — mi_cols
  // stays stale. Very low SL0 bitrate forces a frame drop.
  // After the drop, next SL0 gets last_source = source_last_TL0
  // (640x360 small buffer). SAD loop overruns it → OOB. */
  cfg.g_w = 640;
  cfg.g_h = 360;
  cfg.rc_target_bitrate = 500;
  ASSERT_EQ(aom_codec_enc_config_set(&codec, &cfg), AOM_CODEC_OK);
  {
    int num[] = { 1, 1 }, den[] = { 1, 1 }, br[] = { 1, 499 };
    set_svc(&codec, 2, num, den, br);
  }
  raw = aom_img_alloc(NULL, AOM_IMG_FMT_I420, 640, 360, 1);
  for (int f = 0; f < 20; ++f) {
    aom_svc_layer_id_t lid = { 0, 0 };
    ASSERT_EQ(aom_codec_control(&codec, AV1E_SET_SVC_LAYER_ID, &lid),
              AOM_CODEC_OK);
    fill_image(raw, (uint8_t)((f % 2 == 0) ? 16 : 235));
    encode_one(&codec, raw, pts++);
  }
