...
TEST_F(InvalidFilePoCTest, CrashTest) {
  ...
  static const unsigned char kPoCData[] = {
  ...
  class InMemoryVideoSource : public ::libaom_test::CompressedVideoSource {
   public:
    InMemoryVideoSource(const uint8_t *data, size_t size)
        : data_(data), size_(size), frame_number_(0) {}

    virtual ~InMemoryVideoSource() {}
    ...
    const uint8_t *cxdata() const override {
      return frame_number_ == 0 ? data_ : nullptr;
    }

    size_t frame_size() const override {
      return frame_number_ == 0 ? size_ : 0;
    }
  ...
  InMemoryVideoSource decode_video(kPoCData, kPoCDataLen);
  decode_video.Init();
  ASSERT_NO_FATAL_FAILURE(RunLoop(&decode_video, cfg));
}
