...
using InterpCubicTestParam = InterpCubicRateDistFunc;
...
class InterpCubicTest : public ::testing::TestWithParam<InterpCubicTestParam> {
 public:
  ...
  void SetUp() override { target_func_ = GetParam(); }
...
void InterpCubicTest::CheckOutput() {
  ...
  for (int iter = 0; iter < kNumIter; iter++) {
    ...
    double x = GenerateRandomDouble(0.0000, 1.0000);
...
void InterpCubicTest::SpeedTest() {
  ...
  constexpr int kNumIter = 100000000;
