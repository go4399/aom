 ...
 protected:
  ...
  void PreEncodeFrameHook(::libaom_test::VideoSource *video,
    ...
    if (video->frame() == 0) {
      ...
      // For DeltaQ feature: tests only all pass for aq_mode on. Remove this
      // condition when fixed.
      encoder->Control(AV1E_SET_AQ_MODE,
                       (aq_mode_ || roi_feature_ == kDeltaQ) ? 3 : 0);
...
// The LF feature requires additional changes, so leave it out for now.
