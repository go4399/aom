...
class BaseKeyValAPI : public testing::Test {
 public:
  void SetUp() override {
    ...
    aom_codec_iface_t *iface_cx = aom_codec_av2_cx();
    ...
    aom_codec_iface_t *iface_dx = aom_codec_av2_dx();
...
// key & value API.
#if CONFIG_AV2_ENCODER
class AV2EncValidTest : public BaseKeyValAPI,
                        public testing::WithParamInterface<KeyValParam> {};
class AV2EncInvalidTest : public BaseKeyValAPI,
                          public testing::WithParamInterface<KeyValParam> {};

TEST_P(AV2EncValidTest, Valid) {
...
TEST_P(AV2EncInvalidTest, NullArg) {
  const char *key = std::get<0>(GetParam());
  const char *val = std::get<1>(GetParam());
...
TEST_P(AV2EncInvalidTest, InvalidParam) {
  const char *key = std::get<0>(GetParam());
  const char *val = std::get<1>(GetParam());
...
INSTANTIATE_TEST_SUITE_P(KeyValAPI, AV2EncValidTest,
                         testing::ValuesIn(enc_valid_params));

INSTANTIATE_TEST_SUITE_P(KeyValAPI, AV2EncInvalidTest,
