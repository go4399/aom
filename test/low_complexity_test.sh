...
low_complexity_test_verify_environment() {
  if [ ! "$(av1_encode_available)" = "yes" ] || [ ! "$(av1_decode_available)" = "yes" ]; then
    return 1
  fi
...
low_complexity_mode_test() {
  ...
  local ivf_file="${AOM_TEST_OUTPUT_DIR}/low_complexity_mode_encoder.ivf"
  ...
  local output_file="${AOM_TEST_OUTPUT_DIR}/low_complexity_mode_encoder.y4m"
  ...
  local dec_cmd="$(echo "${perf_prefix}" "${AOM_TEST_PREFIX}" "${decoder}" "${ivf_file}" \
                 -o "${output_file}" ${devnull})"
  echo "$dec_cmd"
  ...
  instruction_count=$( echo "scale=9; ${instruction_count} / 1000.0" | bc )
  echo ${instruction_count}
}

low_complexity_decode_mode_test="low_complexity_mode_test"

run_tests low_complexity_test_verify_environment "${low_complexity_decode_mode_test}"
