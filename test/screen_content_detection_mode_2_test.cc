...
const uint8_t source[] =   {   0,   0,   1,   2, 255,   3,   4,   0,   0,
...
const uint8_t expected[] = {   0,   0, 255, 255, 255, 255, 255,   0,   0,
...
TEST(ScreenContentDetectionMode2, FindDominantValue) {
  ...
  EXPECT_EQ(av1_find_dominant_value(source, width, height, width), 255);
}

TEST(ScreenContentDetectionMode2, DilateBlock) {
  uint8_t dilated[9 * 6] = { 0 };

  av1_dilate_block(source, width, dilated, width, height, width);
