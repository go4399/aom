...
typedef std::tuple<const NnPredict_Func> AV2NnPredictTestParam;

const float epsilon = 1e-3f;  // Error threshold for functional equivalence

class AV2NnPredictTest : public ::testing::TestWithParam<AV2NnPredictTestParam> {
 public:
  AV2NnPredictTest() : weights_buf(nullptr), bias_buf(nullptr) {}
  virtual void SetUp() {
...
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(AV2NnPredictTest);

void AV2NnPredictTest::RunNnPredictTest(const NN_CONFIG *const shape) {
...
void AV2NnPredictTest::RunNnPredictSpeedTest(const NN_CONFIG *const shape,
                                          const int run_times) {
  libaom_test::ClearSystemState();
  float inputs[NN_MAX_NODES_PER_LAYER] = { 0 };
  float outputs_test[NN_MAX_NODES_PER_LAYER] = { 0 };
...
void AV2NnPredictTest::RunNnPredictTest_all(const NN_CONFIG *const shapes,
                                         const int num_shapes) {
  for (int i = 0; i < num_shapes; i++) RunNnPredictTest(&shapes[i]);
}

void AV2NnPredictTest::RunNnPredictSpeedTest_all(const NN_CONFIG *const shapes,
                                              const int num_shapes,
                                              const int run_times) {
  for (int i = 0; i < num_shapes; i++)
    AV2NnPredictTest::RunNnPredictSpeedTest(&shapes[i], run_times);
}

TEST_P(AV2NnPredictTest, RandomValues) {
  RunNnPredictTest_all(shapes, sizeof(shapes) / sizeof(*shapes));
}

TEST_P(AV2NnPredictTest, DISABLED_Speed) {
  RunNnPredictSpeedTest_all(shapes, sizeof(shapes) / sizeof(*shapes), 10000000);
}

#if HAVE_SSE3 && !CONFIG_EXCLUDE_SIMD_MISMATCH
INSTANTIATE_TEST_SUITE_P(SSE3, AV2NnPredictTest,
                         ::testing::Values(av2_nn_predict_sse3));
#endif

#if HAVE_NEON && !CONFIG_EXCLUDE_SIMD_MISMATCH
INSTANTIATE_TEST_SUITE_P(NEON, AV2NnPredictTest,
