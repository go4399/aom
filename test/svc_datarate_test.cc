 ...
 protected:
  ...
  void CheckReconstruction(const aom_image_t &img) {
    ...
    EXPECT_LE(diff, max_allowed_error_)
        ...
        << cfg_.rc_max_quantizer << ": base reconstructed value "
        << (int)base_val << " has error " << diff
        << " which exceeds max allowed" << max_allowed_error_;

    for (unsigned int r = border + sb_size / 2; r < h - border; r += sb_size) {
      for (unsigned int c = border + sb_size / 2; c < w - border;
           c += sb_size) {
        if (y_plane[r * stride + c] != base_val) {
          EXPECT_EQ(y_plane[r * stride + c], base_val)
              ...
              << " (expected base flat value: " << (int)base_val
              << ") for input " << (int)expected_val_ << " and QP "
