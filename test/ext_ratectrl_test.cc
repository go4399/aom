...
int64_t g_bit_count = 0;
int g_actual_encoding_qindex = 0;
...
aom_rc_status_t mock_update_encodeframe_result(
  ...
  g_bit_count = encode_frame_result->bit_count;
  g_actual_encoding_qindex = encode_frame_result->actual_encoding_qindex;
...
class ExtRateCtrlUpdateEncodeFrameResultTest : public ExtRateCtrlTest {
 protected:
  ...
  void FramePktHook(const aom_codec_cx_pkt_t *pkt) override {
    ...
    current_encoder_->Control(AOME_GET_LAST_QUANTIZER, &q_index);
    EXPECT_EQ(q_index, kFrameQindex);
    EXPECT_EQ(g_actual_encoding_qindex, kFrameQindex);
    ...
    // for vis_frame_size.
    const int pkt_size_padding = g_pkt_count_ == 1 ? 0 : 2;
    ...
    EXPECT_EQ((g_bit_count >> 3) + pkt_size_padding,
