 ...
 public:
  void RunTest() {
    ...
    if ((width > 128 || height > 128) &&
        GetParam().TestFunction() != aom_highbd_convolve_copy_c) {
      return;
    }
    const uint16_t *input = FirstRandomInput12(GetParam());
    DECLARE_ALIGNED(32, uint16_t, reference[MAX_SB_SQUARE]);
...
class AV2FeatureDirAccumHighbdTest
 ...
 private:
  ...
  void FillInputBufs() {
    ...
    memset(dir_feature_accum_buf_c, 0, sizeof(dir_feature_accum_buf_c));
    memset(dir_feature_accum_buf_simd, 0, sizeof(dir_feature_accum_buf_simd));
  }
 ...
 private:
  ...
  void TestTskipAccum() {
    ...
    for (int plane = 0; plane < kNumPlanes; ++plane) {
      ...
      const int tskip_lead = PC_WIENER_TSKIP_LEAD_LUMA;
      const int tskip_lag = PC_WIENER_TSKIP_LAG_LUMA;
      memset(tskip_feature_accum_c, 0, sizeof(tskip_feature_accum_c));
      memset(tskip_feature_accum_simd, 0, sizeof(tskip_feature_accum_simd));
