...
static void set_layer_pattern(
  ...
  *use_svc_control = 1;
  ...
  switch (layering_mode) {
     ...
     */
      ...
      if (layer_id->spatial_layer_id == 0) {
        ...
        // Reference LAST and GOLDEN. Set buffer_idx for LAST to slot 0
        // and GOLDEN to slot 1. Update slot 1 (GOLDEN).
        ref_frame_config->ref_idx[SVC_LAST_FRAME] = 0;
  ...
  for (i = 0; i < INTER_REFS_PER_FRAME; i++) {
    if (ref_frame_config->reference[i] == 1) {
      *is_reference_frame = 1;
      break;
    }
  }
}
  ...
  while (frame_avail || got_data) {
    ...
    for (int slx = 0; slx < ss_number_layers; slx++) {
      ...
      if (got_data && app_input.decode) {
        // Look for mismatch for frames that are used as a reference.
