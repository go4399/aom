  ...
  switch (layering_mode) {
    case 0:
      if (use_rps_example == 0) {
        ...
        // FIX: to refresh a buffer slot: assign a reference frame (use LAST
        // here) to that slot.
        ref_frame_config->ref_idx[SVC_LAST2_FRAME] = last_idx_refresh;

        if (!is_key_frame) {
          // Use one reference (LAST) newest_available_ref_poc frames
          // behine current frame.
          ref_frame_config->ref_idx[SVC_LAST_FRAME] =
              newest_available_ref_poc % slot_number;
          ref_frame_config->reference[SVC_LAST_FRAME] = 1;

          // Use additional reference (GOLDEN) (1 + newest_available_ref_poc) frames
          // behine current frame. And compound prediciton also enabled above.
          if (newest_available_ref_poc > 1) {
            ref_frame_config->ref_idx[SVC_GOLDEN_FRAME] =
                (newest_available_ref_poc - 1) % slot_number;
