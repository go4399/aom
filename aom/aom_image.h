 ...
 *
 * During decoding, when the library encounters a metadata OBU, it is flagged
 * as either AOM_MIF_ANY_FRAME or AOM_MIF_ANY_FRAME_LAYER_SPECIFIC (if it was
 * added using a *_LAYER_SPECIFC insert flag AND the video contains multiple
 * layers) and emitted with the next output aom_image.
 */
