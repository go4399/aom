...
typedef struct aom_codec_enc_cfg {
   ...
   *
   * This value identifies the actual bit_depth of the input source in bits.
   * Note that the frames passed as input to the encoder must match codec bit
   * depth. If there is a mismatch between source bit depth and codec bit depth,
   * then application is required to match the precision before passing the
   * frame for encoding. Additionally, this variable is used by the library to
   * compute quality metrics at source bit depth. So, source bitdepth must not
   * exceed codec bit depth.
   */
