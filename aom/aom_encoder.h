...
typedef struct aom_codec_enc_cfg {
   ...
   * bit-depth, then application is required to upshift the frame to the codec
   * bit-depth before passing it for encoding. Additionally, this variable is
   * used by the library to compute quality metrics at source bit-depth. So,
