...
#ifndef AV2_AV2_COMMON_MV_H_
#define AV2_AV2_COMMON_MV_H_
...
static INLINE void get_phase_from_mv(MV ref_mv, MV *sub_mv_offset,
...
static inline int compression_mv(int16_t val) {
  ...
  const int step_log2 = AOMMAX(0, get_msb_signed(val) - 4);
...
static inline int uncompression_mv(int16_t val) {
  const int abs_val = abs(val);
  const int step_log2 = AOMMAX(0, (abs_val >> 4) - 1);
...
static INLINE void process_mv_for_tmvp(MV *mv) {
  mv->row = compression_mv(mv->row);
  mv->col = compression_mv(mv->col);
}

// Uncompress TMVP MVs
static INLINE void fetch_mv_from_tmvp(MV *mv) {
