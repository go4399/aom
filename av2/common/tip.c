...
static INLINE void tip_build_inter_predictors_8x8(
...
static INLINE void tip_build_inter_predictors_8x8_and_bigger(
...
static INLINE void tip_component_build_inter_predictors(
    const AV2_COMMON *cm, MACROBLOCKD *xd, int plane, TIP_PLANE *tip_plane,
    const MV mv[2], int bw, int bh, int mi_x, int mi_y, uint16_t **mc_buf,
    CONV_BUF_TYPE *tmp_conv_dst, CalcSubpelParamsFunc calc_subpel_params_func) {
  tip_build_inter_predictors_8x8_and_bigger(
...
static INLINE void tip_setup_pred_plane(struct buf_2d *dst, void *src,
  ...
  dst->buf = (uint16_t *)src + scaled_buffer_offset(x, y, stride, scale);
  dst->buf0 = src;
  dst->width = width;
  dst->height = height;
...
static INLINE void tip_component_setup_dst_planes(AV2_COMMON *const cm,
                                                  TIP_PLANE *const tip_plane,
                                                  const int plane,
                                                  const int tpl_row,
                                                  const int tpl_col) {
...
static INLINE void tip_setup_tip_frame_planes(
    AV2_COMMON *cm, MACROBLOCKD *xd, int blk_row_start, int blk_col_start,
