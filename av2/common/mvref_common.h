 ...
 */
#ifndef AV2_AV2_COMMON_MVREF_COMMON_H_
#define AV2_AV2_COMMON_MVREF_COMMON_H_
...
static INLINE int get_mf_sb_size_log2(int sb_size, int mib_size_log2,
...
static INLINE int get_block_position(const AV2_COMMON *cm, int *mi_r, int *mi_c,
...
static INLINE void get_proc_size_and_offset(AV2_COMMON *cm) {
...
static INLINE int check_block_position(const AV2_COMMON *cm, int row, int col,
...
static INLINE int get_block_position_no_constraint(const AV2_COMMON *cm,
                                                   int *mi_r, int *mi_c,
                                                   int blk_row, int blk_col,
                                                   MV mv, int sign_bias) {
  // The motion vector in units of 1/8-pel
...
static INLINE aom_cdf_prob *av2_get_drl_cdf(const MB_MODE_INFO *const mbmi,
...
static INLINE aom_cdf_prob *av2_get_warp_ref_idx_cdf(FRAME_CONTEXT *ec_ctx,
                                                     int bit_idx) {
  const int ctx = 0;
...
static INLINE void get_mv_projection_clamp(MV *output, MV ref, int num, int den,
...
static INLINE void tip_get_mv_projection(MV *output, MV ref, int scale_factor) {
...
static INLINE int derive_col_mv_tpl_offset(const AV2_COMMON *const cm,
                                           int blk_tpl_col) {
...
static INLINE int derive_row_mv_tpl_offset(const AV2_COMMON *const cm,
                                           int blk_tpl_row) {
  const int frame_mvs_tpl_cols =
      ROUND_POWER_OF_TWO(cm->mi_params.mi_cols, TMVP_SHIFT_BITS);
...
static INLINE int derive_block_mv_tpl_offset(const AV2_COMMON *const cm,
...
static INLINE void get_tip_mv(const AV2_COMMON *cm, const MV *block_mv,
...
static INLINE void derive_non_tip_mode_smvp_from_tip(const AV2_COMMON *cm,
...
#endif  // AV2_AV2_COMMON_MVREF_COMMON_H_
