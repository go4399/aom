...
#include "aom_dsp/av2_grain_synthesis.h"
...
typedef struct BufferRemovalTimingInfo {
...
typedef struct SequenceHeader {
  ...
  av2_opfl_refine_type enable_opfl_refine;  // optical flow refinement type for
                                            // this frame
...
typedef struct AV2Common {
   ...
   * Sub-stream IDs
   */
  int stream_ids[AV2_MAX_NUM_STREAMS];
  /*!
...
static INLINE bool is_at_sb_top_boundary(int mi_row, int mib_size) {
...
static INLINE int is_sdp_enabled_in_keyframe(const AV2_COMMON *const cm) {
...
static INLINE bool is_bsize_above_decoupled_thresh(BLOCK_SIZE bsize) {
  return bsize >= BLOCK_64X64 && bsize <= BLOCK_LARGEST;
}

// Whether the partition tree contains a block size that is strictly smaller
// than width x height.
static INLINE bool tree_has_bsize_smaller_than(const PARTITION_TREE *ptree,
                                               int width, int height) {
...
static INLINE bool is_sdp_share_partition(PARTITION_TYPE luma_partition,
...
static INLINE bool is_luma_chroma_share_same_partition(
...
static INLINE CFL_ALLOWED_FOR_SDP_TYPE
...
static INLINE CFL_ALLOWED_FOR_SDP_TYPE is_cfl_allowed_for_sdp(
...
static INLINE bool is_partition_implied_at_boundary(
 ...
 */
static INLINE PARTITION_TYPE av2_get_normative_forced_partition_type(
...
static INLINE void get_chroma_ref_offsets(BLOCK_SIZE bsize,
                                          PARTITION_TYPE partition,
...
static INLINE bool is_chroma_ref_within_boundary(
...
static INLINE void init_allowed_partitions_for_signaling(
...
static INLINE bool is_do_split_implied(const bool *partition_allowed,
...
static INLINE void av2_set_frame_sb_size(AV2_COMMON *cm, BLOCK_SIZE sb_size) {
...
static INLINE int opfl_allowed_cur_refs_bsize(const AV2_COMMON *cm,
  ...
  if (cm->seq_params.enable_opfl_refine == AV2_OPFL_REFINE_NONE ||
...
static INLINE bool disable_opfl_for_16x16_tip_ref(TIP_FRAME_MODE tip_frame_mode,
                                                  int bw, int bh,
...
// case.
static INLINE bool disable_opfl_for_tip_direct(TIP_FRAME_MODE tip_frame_mode,
                                               InterpFilter tip_interp_filter,
                                               int enable_tip_refinemv) {
...
static INLINE BLOCK_SIZE get_tip_bsize_from_bw_bh(int bw, int bh) {
  assert(bh == 16 || bh == 8);
  if (bh == 16) {
...
static INLINE BLOCK_SIZE get_unit_bsize_for_tip_ref(
...
static INLINE BLOCK_SIZE get_unit_bsize_for_tip_frame(
...
static INLINE int is_optflow_refinement_enabled(const AV2_COMMON *cm,
                                                const MACROBLOCKD *xd,
                                                const MB_MODE_INFO *mi,
                                                int plane, int tip_ref_frame) {
  if (cm->seq_params.enable_opfl_refine == AV2_OPFL_REFINE_NONE ||
      cm->features.opfl_refine_type == REFINE_NONE)
    return 0;
...
static INLINE aom_codec_err_t av2_get_chroma_subsampling(
...
static INLINE int is_frame_tile_config_reuse_eligible(
  ...
  if (AOMMIN(tile_width_sb, last_tile_width_sb) < 1) return 0;
  ...
  if (AOMMIN(tile_height_sb, last_tile_height_sb) < 1) return 0;
...
static INLINE void av2_initialize_ci_params(ContentInterpretation *ci_params) {
  ci_params->ci_chroma_sample_position[0] = AOM_CSP_UNKNOWN;
  ci_params->ci_chroma_sample_position[1] = AOM_CSP_UNKNOWN;
  ci_params->color_info.color_description_idc = AV2_COLOR_DESC_IDC_EXPLICIT;
