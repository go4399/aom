...
static double get_max_compressed_size(const AV2LevelSpec *const level_spec,
  ...
  return AOMMIN(limit, max_compressed_size);
}
...
int av2_get_max_level_ref_frames(const AV2_COMMON *const cm, OBU_TYPE obu_type,
  ...
  const int max_level_ref_frames = (int)AOMMIN(cap, limit);
...
static void av2_decoder_model_start_frame_decode(
  ...
  if (!show_existing_frame) {
    ...
    int max_tile_limit = AOMMIN(level_spec->max_tiles, AOMMAX(1, scaled));
...
static void av2_decoder_model_check_output_frame(
  ...
  if (presentation_time >= 0.0 && previous_presentation_time >= 0.0 &&
    ...
    const double min_interval = AOMMAX(
  ...
  *max_tile_width = 0;
  ...
  for (int tile_row = 0; tile_row < tile_rows; ++tile_row) {
    for (int tile_col = 0; tile_col < tile_cols; ++tile_col) {
      ...
      *max_tile_width = AOMMAX(*max_tile_width, tile_width);
...
void av2_update_level_info(AV2_COMP *cpi, size_t size, int64_t ts_start,
  ...
  for (int i = 0; i < seq_params->operating_points_cnt_minus_1 + 1; ++i) {
    ...
    level_stats->max_tile_width =
        AOMMAX(level_stats->max_tile_width, max_tile_width);
...
aom_codec_err_t av2_get_seq_level_idx(const AV2_COMP *cpi,
