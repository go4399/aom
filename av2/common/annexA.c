...
static int check_bit_depth_8_10(int bit_depth) {
  if (bit_depth != AOM_BITS_8 && bit_depth != AOM_BITS_10) {
    return 0;
  }
  return 1;
}

static aom_codec_err_t check_chroma_format(int monochrome, int is_420,
...
static aom_codec_err_t check_mlayer_count(int profile_idc, int seq_max_mcount) {
...
int av2_check_profile_interop_conformance(
    ...
    struct aom_internal_error_info *error_info, int is_decoder) {
...
#if CONFIG_TESTONLY_12BIT_SUPPORT
  if (profile == TEST_ONLY_12BIT_PROFILE && bit_depth == AOM_BITS_12) return 1;
#endif  // CONFIG_TESTONLY_12BIT_SUPPORT
  ...
  switch (profile) {
    ...
    case MAIN_420_10_IP2:
      ...
      if (err != AOM_CODEC_OK) {
        aom_internal_error(
            error_info,
            is_decoder ? AOM_CODEC_UNSUP_BITSTREAM : AOM_CODEC_INVALID_PARAM,
