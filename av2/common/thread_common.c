...
void av2_tip_dealloc(AV2TipSync *const tip_sync) {
  if (tip_sync != NULL) {
    ...
    if (tip_sync->job_mutex != NULL) {
      ...
      aom_free(tip_sync->job_mutex);
    }
#endif  // CONFIG_MULTITHREAD

    aom_free(tip_sync->job_queue);
    // clear the structure as the source of this call may be a resize in which
    // case this call will be followed by an _alloc() which may fail.
    av2_zero(*tip_sync);
  }
...
static void tip_alloc(AV2TipSync *const tip_sync, AV2_COMMON *cm,
  ...
  CHECK_MEM_ERROR(cm, tip_sync->job_mutex,
                  aom_malloc(sizeof(*(tip_sync->job_mutex))));
  ...
  CHECK_MEM_ERROR(
      ...
      aom_malloc(sizeof(*(tip_sync->job_queue)) * (mvs_rows / step)));
}
...
static INLINE void reset_cdef_job_info(AV2CdefSync *cdef_sync) {
...
static INLINE int get_cdef_row_next_job(AV2CdefSync *cdef_sync, int *cur_fbr,
                                        const int nvfb) {
#if CONFIG_MULTITHREAD
