...
/* This function test the reference frame used for inter prediction.
   ...
   BRU reference frame.
*/
static INLINE int bru_is_valid_inter(const AV2_COMMON *const cm,
...
static INLINE bool is_bru_not_active_and_not_on_partial_border(
...
static INLINE int bru_get_num_of_active_region(const AV2_COMMON *const cm) {
  if (cm->bru.enabled) {
...
static INLINE bool bru_active_map_validation(AV2_COMMON *cm) {
  ...
  AV2PixelRect *regions =
      (AV2PixelRect *)aom_calloc(max_regions, sizeof(AV2PixelRect));
