...
static INLINE int is_tip_block_with_mv_refinement(const AV2_COMMON *const cm,
  ...
  if (cm->features.tip_frame_mode == TIP_FRAME_AS_OUTPUT) {
    ...
    if (cm->seq_params.enable_opfl_refine == AV2_OPFL_REFINE_NONE) return 0;
  }
...
static INLINE bool is_tip_coded_as_16x16_block(const AV2_COMMON *cm,
...
static INLINE void setup_pred_planes_for_tip(const TIP *tip_ref,
                                             MACROBLOCKD *xd, int plane_start,
                                             int plane_end, int mi_col,
                                             int mi_row) {
...
static INLINE bool is_subblock_outside(int x, int y, int mi_cols, int mi_rows,
