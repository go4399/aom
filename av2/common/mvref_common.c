...
static INLINE void fill_mvp_from_derived_smvp(
...
static INLINE void derive_ref_mv_candidate_from_tip_mode(
...
static INLINE void add_ref_mv_candidate_ctx(const MB_MODE_INFO *const candidate,
...
static INLINE int is_valid_candidate(const MACROBLOCKD *xd, int mi_row,
                                     int mi_col, int row_offset, int col_offset,
                                     int check_col_offset) {
...
static INLINE void scan_blk_mbmi_ctx(
...
static INLINE int compute_cur_to_ref_dist(const AV2_COMMON *cm,
                                          MV_REFERENCE_FRAME ref_frame) {
...
static INLINE int get_rmb_list_index(const MV_REFERENCE_FRAME ref_frame) {
  MV_REFERENCE_FRAME rf[2];
  av2_set_ref_frame(rf, ref_frame);
...
static INLINE bool check_rmb_cand(
...
static INLINE bool add_to_ref_bv_list(CANDIDATE_MV cand_mv,
...
static INLINE void add_tmvp_candidate(
...
static INLINE int assign_tmvp_high_priority(const AV2_COMMON *cm,
                                            MV_REFERENCE_FRAME rf[2]) {
...
static INLINE void add_derived_smvp_candidates(
...
static INLINE void add_ref_mv_bank_candidates(
    const AV2_COMMON *cm, const MACROBLOCKD *xd, MV_REFERENCE_FRAME *rf,
...
static INLINE int compute_aligned_offset(int mi_col, int col_offset) {
...
static INLINE int is_above_smvp_available(const MACROBLOCKD *xd, int mi_col,
                                          int col_offset) {
...
static INLINE void get_row_smvp_states(const AV2_COMMON *cm,
                                       const MACROBLOCKD *xd,
...
static INLINE int fill_warp_corner_projected_point(
...
static INLINE int generate_points_from_corners(
...
static INLINE POSITION get_pos_from_pos_idx(const AV2_COMMON *cm,
                                            const MACROBLOCKD *xd,
                                            int pos_idx) {
  MVP_UNIT_STATUS row_smvp_state[4] = { 0 };
...
static INLINE int get_cand_from_pos_idx(const AV2_COMMON *cm,
                                        const MACROBLOCKD *xd, int pos_idx) {
  MVP_UNIT_STATUS row_smvp_state[4] = { 0 };
  if (pos_idx == 2 || pos_idx == 4 || pos_idx == 6 || pos_idx == 7) {
    get_row_smvp_states(cm, xd, row_smvp_state);
  }
...
static INLINE int check_pos_and_get_base_pos(const AV2_COMMON *cm,
                                             const MACROBLOCKD *xd,
                                             const MB_MODE_INFO *mbmi,
                                             POSITION *base_pos, int pos_idx) {
  const TileInfo *const tile = &xd->tile;
