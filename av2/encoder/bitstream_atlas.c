...
static uint32_t write_ats_region_info(struct AtlasRegionInfo *atlas_reg_params,
  ...
  av2_wb_write_uvlc(wb, atlas_reg_params->ats_num_region_columns_minus_1);
  av2_wb_write_uvlc(wb, atlas_reg_params->ats_num_region_rows_minus_1);

  av2_wb_write_bit(wb, atlas_reg_params->ats_uniform_spacing_flag);
  if (!atlas_reg_params->ats_uniform_spacing_flag) {
    for (int i = 0; i < atlas_reg_params->ats_num_region_columns_minus_1 + 1;
         i++) {
      av2_wb_write_uvlc(wb, atlas_reg_params->ats_column_width_minus_1[i]);
    }
    for (int i = 0; i < atlas_reg_params->ats_num_region_rows_minus_1 + 1;
         i++) {
      av2_wb_write_uvlc(wb, atlas_reg_params->ats_row_height_minus_1[i]);
    }
  } else {
    av2_wb_write_uvlc(wb, atlas_reg_params->ats_region_width_minus_1);
    av2_wb_write_uvlc(wb, atlas_reg_params->ats_region_height_minus_1);
  }
  return 0;
}

static uint32_t write_ats_multistream_alpha_atlas_info(
    struct AtlasBasicInfo *ats_basic_info, struct aom_write_bit_buffer *wb) {
  av2_wb_write_uvlc(wb, ats_basic_info->ats_atlas_width);
  av2_wb_write_uvlc(wb, ats_basic_info->ats_atlas_height);
  av2_wb_write_uvlc(wb, ats_basic_info->ats_num_atlas_segments_minus_1);

  av2_wb_write_bit(wb, ats_basic_info->ats_alpha_segments_present_flag);
  av2_wb_write_bit(wb, ats_basic_info->ats_background_info_present_flag);
  if (ats_basic_info->ats_background_info_present_flag == 1) {
    av2_wb_write_literal(wb, ats_basic_info->ats_background_red_value, 8);
    av2_wb_write_literal(wb, ats_basic_info->ats_background_green_value, 8);
    av2_wb_write_literal(wb, ats_basic_info->ats_background_blue_value, 8);
  }

  const int NumSegments = ats_basic_info->ats_num_atlas_segments_minus_1 + 1;
  for (int i = 0; i < NumSegments; i++) {
    av2_wb_write_literal(wb, ats_basic_info->ats_input_stream_id[i], 5);
    av2_wb_write_uvlc(wb, ats_basic_info->ats_segment_top_left_pos_x[i]);
    av2_wb_write_uvlc(wb, ats_basic_info->ats_segment_top_left_pos_y[i]);
    av2_wb_write_uvlc(wb, ats_basic_info->ats_segment_width[i]);
    av2_wb_write_uvlc(wb, ats_basic_info->ats_segment_height[i]);
    if (ats_basic_info->ats_alpha_segments_present_flag == 1 &&
        i < NumSegments - 1)
      av2_wb_write_bit(wb, ats_basic_info->ats_alpha_segment_flag[i]);
  }
  return 0;
}
...
static uint32_t write_ats_basic_info(struct AtlasBasicInfo *ats_basic_info,
                                     struct aom_write_bit_buffer *wb) {
  av2_wb_write_bit(wb, ats_basic_info->ats_stream_id_present);
...
static uint32_t write_ats_region_to_segment_mapping(
  ...
  if (!ats_reg_seg_map->ats_single_region_per_atlas_segment_flag) {
    av2_wb_write_uvlc(wb, ats_reg_seg_map->ats_num_atlas_segments_minus_1);
    const int NumSegments = ats_reg_seg_map->ats_num_atlas_segments_minus_1 + 1;
    for (int i = 0; i < NumSegments; i++) {
      av2_wb_write_uvlc(wb, ats_reg_seg_map->ats_top_left_region_column[i]);
      av2_wb_write_uvlc(wb, ats_reg_seg_map->ats_top_left_region_row[i]);
...
static uint32_t write_ats_label_segment_info(
  ...
  av2_wb_write_bit(wb, ats_label->ats_signalled_atlas_segment_ids_flag);
  if (ats_label->ats_signalled_atlas_segment_ids_flag) {
    for (int i = 0; i < NumSegments; i++) {
      av2_wb_write_literal(wb, ats_label->ats_atlas_segment_id[i],
...
int av2_write_atlas_segment_info(struct AtlasSegmentInfo *atlas_params,
                                 struct aom_write_bit_buffer *wb) {
  av2_wb_write_literal(wb, atlas_params->atlas_segment_id, 3);
  av2_wb_write_uvlc(wb, atlas_params->atlas_segment_mode_idc);
  ...
  if (atlas_params->atlas_segment_mode_idc == ENHANCED_ATLAS) {
    ...
    num_segments = 1;
    av2_wb_write_uvlc(wb, atlas_params->ats_nominal_width_minus1);
    av2_wb_write_uvlc(wb, atlas_params->ats_nominal_height_minus1);
...
uint32_t av2_write_atlas_segment_info_obu(AV2_COMP *cpi, uint8_t *const dst) {
  ...
  av2_wb_write_bit(&wb, atlas_params->ats_extension_present_flag);
