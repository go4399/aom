...
void av2_rd_pick_palette_intra_sby(
  ...
  // In AV2 buf_2d.buf is already a real uint16_t* (always-16-bit internal
  // storage); it is not a tagged byte pointer, so no CONVERT_TO_SHORTPTR.
  const uint16_t *src_unshifted = src;
