...
void av2_init_seq_coding_tools(AV2_COMP *cpi, SequenceHeader *seq,
  ...
  if (level_idx < SEQ_LEVELS) {
    ...
    if (max_level_pic_size > 0 && cur_pic_size > 0) {
      ...
      const int max_dpb =
          (int)AOMMIN(default_dpb_size << explicit_num_ref_frames,
...
static void set_content_interpreation_params(struct AV2_COMP *cpi,
  ...
  if (ci_params->color_info.color_primaries == AOM_CICP_CP_BT_709 &&
      ...
      ci_params->color_info.matrix_coefficients == AOM_CICP_MC_BT_709) {
    ci_params->color_info.color_description_idc = AV2_COLOR_DESC_IDC_BT709SDR;
    ...
    ci_params->color_info.color_description_idc = AV2_COLOR_DESC_IDC_SRGB;
  } else if (ci_params->color_info.color_primaries == AOM_CICP_CP_BT_709 &&
             ci_params->color_info.transfer_characteristics ==
                 AOM_CICP_TC_SRGB &&
             ci_params->color_info.matrix_coefficients ==
                 AOM_CICP_MC_BT_470_B_G) {
    ci_params->color_info.color_description_idc = AV2_COLOR_DESC_IDC_SYCC;
  } else if (ci_params->color_info.color_primaries == AOM_CICP_CP_BT_2020 &&
             ci_params->color_info.transfer_characteristics ==
                 AOM_CICP_TC_SMPTE_2084 &&
             ci_params->color_info.matrix_coefficients ==
                 AOM_CICP_MC_BT_2020_NCL) {
    ci_params->color_info.color_description_idc = AV2_COLOR_DESC_IDC_BT2100PQ;
  } else if (ci_params->color_info.color_primaries == AOM_CICP_CP_BT_2020 &&
             ci_params->color_info.transfer_characteristics ==
                 AOM_CICP_TC_HLG &&
             ci_params->color_info.matrix_coefficients ==
                 AOM_CICP_MC_BT_2020_NCL) {
    ci_params->color_info.color_description_idc = AV2_COLOR_DESC_IDC_BT2100HLG;
  }
  ci_params->color_info.full_range_flag = color_cfg->color_range;

  if (ci_params->color_info.color_description_idc ==
          AV2_COLOR_DESC_IDC_EXPLICIT &&
    ...
    ci_params->ci_color_description_present_flag = 0;
             ...
             (ci_params->color_info.color_description_idc ==
              AV2_COLOR_DESC_IDC_BT709SDR) ||
             (ci_params->color_info.color_description_idc ==
              AV2_COLOR_DESC_IDC_SRGB) ||
             (ci_params->color_info.color_description_idc ==
              AV2_COLOR_DESC_IDC_SYCC) ||
             (ci_params->color_info.color_description_idc ==
              AV2_COLOR_DESC_IDC_BT2100PQ) ||
             (ci_params->color_info.color_description_idc ==
              AV2_COLOR_DESC_IDC_BT2100HLG) ||
             (ci_params->color_info.full_range_flag != 0)) {
  ...
  if (chroma_format_idc == CHROMA_FORMAT_422 ||
    ...
    ci_params->ci_chroma_sample_position_present_flag =
        color_cfg->chroma_sample_position != AOM_CSP_UNKNOWN;
...
static void init_config(struct AV2_COMP *cpi, AV2EncoderConfig *oxcf) {
  ...
  if (!av2_check_profile_interop_conformance(seq_params, &cm->error, 0)) {
    ...
    aom_internal_error(
        &cm->error, AOM_CODEC_INVALID_PARAM,
...
AV2_COMP *av2_create_compressor(AV2EncoderConfig *oxcf, BufferPool *const pool,
  ...
  AV2_COMP *volatile const cpi = aom_memalign(32, sizeof(AV2_COMP));
...
// to reset() is made.
static INLINE void terminate_worker_data(AV2_COMP *cpi) {
...
static INLINE void free_thread_data(AV2_COMP *cpi) {
  MultiThreadInfo *const mt_info = &cpi->mt_info;
...
static void generate_psnr_packet(AV2_COMP *cpi) {
  ...
  aom_calc_highbd_psnr(cpi->source, b, &psnr, bit_depth, in_bit_depth,
  ...
  for (ref_frame = 0; ref_frame < INTER_REFS_PER_FRAME; ++ref_frame) {
    ...
    if (buf != NULL) {
      ...
      av2_setup_scale_factors_for_frame(sf, buf->buf.y_crop_width,
                                        ...
                                        cm->height);
      if (av2_is_scaled(sf)) aom_extend_frame_borders(&buf->buf, num_planes);
    }
  }
...
static INLINE int compute_tip_direct_output_mode_RD(AV2_COMP *cpi,
  ...
  if (allow_tip_direct_output(cm)) {
    ...
    av2_finalize_encoded_frame(cpi);
    if (av2_pack_bitstream(cpi, dest, size, largest_tile_id) != AOM_CODEC_OK)
      return AOM_CODEC_ERROR;
...
int av2_receive_raw_frame(AV2_COMP *cpi, aom_enc_frame_flags_t frame_flags,
...
aom_codec_err_t av2_copy_new_frame_enc(AV2_COMMON *cm,
...
void av2_apply_encoding_flags(AV2_COMP *cpi, aom_enc_frame_flags_t flags) {
