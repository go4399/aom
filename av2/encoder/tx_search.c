...
static INLINE void fetch_tx_rd_info(int n4, const MB_RD_INFO *const tx_rd_info,
...
static INLINE void save_tx_rd_info(int n4, uint32_t hash,
...
static INLINE void PrintPredictionUnitStats(const AV2_COMP *const cpi,
...
static INLINE void inverse_transform_block_facade(
...
static unsigned pixel_dist_visible_only(
  ...
  if (txb_rows == visible_rows && txb_cols == visible_cols) {
    const int tx_bsize = enc_txsize_to_bsize[tx_size];
    unsigned sse;
    cpi->fn_ptr[tx_bsize].vf((const uint8_t *)src, src_stride,
                             (const uint8_t *)dst, dst_stride, &sse);
    return sse;
  }
...
static void search_cctx_type(const AV2_COMP *cpi, MACROBLOCK *x, int block,
  ...
  TxfmParam txfm_param;
  av2_setup_xform(cm, x, AOM_PLANE_U, tx_size, tx_type, CCTX_NONE, &txfm_param);
  QUANT_PARAM quant_param;
  int xform_quant_idx =
      skip_trellis
          ? (USE_B_QUANT_NO_TRELLIS ? AV2_XFORM_QUANT_B : AV2_XFORM_QUANT_FP)
  ...
  if (!uv_coeffs_available) {
    av2_xform(x, AOM_PLANE_U, block, blk_row, blk_col, plane_bsize, &txfm_param,
              0, NULL);
    av2_xform(x, AOM_PLANE_V, block, blk_row, blk_col, plane_bsize, &txfm_param,
              0, NULL);
    if (av2_use_qmatrix(&cm->quant_params, xd, mbmi->segment_id))
      av2_setup_qmatrix(&cm->quant_params, xd, AOM_PLANE_U, tx_size, tx_type,
                        &quant_param);
    av2_quant(x, AOM_PLANE_U, block, &txfm_param, &quant_param);
    if (av2_use_qmatrix(&cm->quant_params, xd, mbmi->segment_id))
      av2_setup_qmatrix(&cm->quant_params, xd, AOM_PLANE_V, tx_size, tx_type,
                        &quant_param);
    av2_quant(x, AOM_PLANE_V, block, &txfm_param, &quant_param);
  }
...
static INLINE void tx_type_rd(const AV2_COMP *cpi, MACROBLOCK *x,
                              TX_SIZE tx_size, int blk_row, int blk_col,
                              int block, int plane_bsize, TXB_CTX *txb_ctx,
                              RD_STATS *rd_stats, FAST_TX_SEARCH_MODE ftxs_mode,
                              int64_t ref_rdcost, TXB_RD_INFO *rd_info_array) {
  (void)rd_info_array;

  av2_subtract_txb(x, AOM_PLANE_Y, plane_bsize, blk_col, blk_row, tx_size,
...
static INLINE void choose_largest_tx_size(const AV2_COMP *const cpi,
...
static INLINE void choose_lossless_tx_size(const AV2_COMP *const cpi,
                                           MACROBLOCK *x, RD_STATS *rd_stats,
                                           int64_t ref_best_rd, BLOCK_SIZE bs) {
  MACROBLOCKD *const xd = &x->e_mbd;
  MB_MODE_INFO *const mbmi = xd->mi[0];
...
static void choose_tx_size_type_from_rd(const AV2_COMP *const cpi,
  ...
  for (TX_PARTITION_TYPE type = 0; type < TX_PARTITION_TYPES; ++type) {
    ...
    if (cur_rd < best_rd) {
      av2_copy_array(best_blk_skip, txfm_info->blk_skip[AOM_PLANE_Y], num_blks);
  ...
  if (rd_stats->rate != INT_MAX) {
    ...
    av2_copy_array(txfm_info->blk_skip[AOM_PLANE_Y], best_blk_skip, num_blks);
  }
}
...
static INLINE void block_rd_txfm_joint_uv(int dummy_plane, int block,
                                          int blk_row, int blk_col,
                                          BLOCK_SIZE plane_bsize,
...
void av2_txfm_rd_joint_uv(MACROBLOCK *x, const AV2_COMP *cpi,
  ...
  // for U and V
  av2_foreach_transformed_block_in_plane(xd, plane_bsize, AOM_PLANE_U,
...
static INLINE int model_based_tx_search_prune(const AV2_COMP *cpi,
