...
/*!\brief Algorithm for AV2 loop filter level selection.
 *
 ...
 * \arg \c apply_deblocking_filter[0]: the vertical filter apply flag for Y
 * plane \arg \c apply_deblocking_filter[1]: the horizontal filter apply flag
 * for Y plane \arg \c apply_deblocking_filter_u: the filter apply flag for U
 * plane \arg \c apply_deblocking_filter_v: the filter apply flag for V plane
 *
 * \n
 * \b Overview
 * \par
 ...
 * In AV2, for each frame, we employ the four filter levels, based on these
 * observations: \n
 ...
 * Let vertical filter level (apply_deblocking_filter[0]) and the horizontal
 * filter level (apply_deblocking_filter[1]) be equal to it. \n Keep the
 * horizontal filter level the same and search and determine the vertical filter
 * level. \n Search and determine the horizontal filter level. \n Search and
 * determine filter level for U plane. \n Search and determine filter level for
 * V plane.
 *
 * \par
 ...
 *
 */
void av2_pick_filter_level(const struct yv12_buffer_config *sd,
                           struct AV2_COMP *cpi, LPF_PICK_METHOD method);
