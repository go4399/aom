  ...
  for (int ref_idx = 0; ref_idx < num_refs; ref_idx++) {
    ...
    if (cm->ref_frame_flags & (1 << ref)) {
      ...
      curr_var = cpi->fn_ptr[bsize].vf(
          (const uint8_t *)x->plane[0].src.buf, x->plane[0].src.stride,
          (const uint8_t *)xd->plane[0].dst.buf, xd->plane[0].dst.stride,
          &curr_sse);
...
static void compute_sms_data(AV2_COMP *const cpi, const TileInfo *const tile,
  ...
  if (cm->ref_frame_flags & (1 << ref_frame)) {
    ...
    for (int idx = 0; idx < sms_data->num_start_mvs; idx++) {
      ...
      sms_data->var = cpi->fn_ptr[bsize].vf((const uint8_t *)src_buf, src_stride,
                                            (const uint8_t *)dst_buf, dst_stride,
                                            &sms_data->sse);
#if CONFIG_ML_PART_SPLIT
      if (need_residual_stats) {
...
static INLINE int64_t clip_rate(const int rate) {
  if (rate == INT_MAX) {
