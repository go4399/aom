...
static int64_t av2_get_sse_plane_available(
    const YV12_BUFFER_CONFIG *a, const YV12_BUFFER_CONFIG *b, int plane,
    const uint8_t *active_mode_map, int active_map_stride, int cols, int rows,
    int unit_width, int unit_height) {
  int64_t total_sse = 0;
  const int a_stride = plane == 0 ? a->y_stride : a->uv_stride;
  const int b_stride = plane == 0 ? b->y_stride : b->uv_stride;
  const uint16_t *a_buf =
      (const uint16_t *)(plane == 0 ? a->y_buffer
                                    : (plane == 1 ? a->u_buffer : a->v_buffer));
  const uint16_t *b_buf =
      (const uint16_t *)(plane == 0 ? b->y_buffer
                                    : (plane == 1 ? b->u_buffer : b->v_buffer));

  for (int r = 0; r < rows; r++) {
    for (int c = 0; c < cols; c++) {
      if (active_mode_map[r * active_map_stride + c] == 2) {
        const int y_start = r * unit_height;
        const int x_start = c * unit_width;
        const int w =
            AOMMIN(unit_width,
                   (plane == 0 ? a->y_crop_width : a->uv_crop_width) - x_start);
        const int h = AOMMIN(
            unit_height,
            (plane == 0 ? a->y_crop_height : a->uv_crop_height) - y_start);
        if (w > 0 && h > 0) {
          const uint16_t *pa = a_buf + y_start * a_stride + x_start;
          const uint16_t *pb = b_buf + y_start * b_stride + x_start;
          for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
              const int diff = pa[j] - pb[j];
              total_sse += diff * diff;
            }
            pa += a_stride;
            pb += b_stride;
...
static void yv12_copy_plane(const YV12_BUFFER_CONFIG *src_bc,
  ...
  switch (plane) {
    case 0: av2_yv12_copy_y(src_bc, dst_bc); break;
    case 1: av2_yv12_copy_u(src_bc, dst_bc); break;
    case 2: av2_yv12_copy_v(src_bc, dst_bc); break;
...
static int64_t try_filter_frame(const YV12_BUFFER_CONFIG *sd,
  ...
  if (cm->bru.enabled) {
    filt_err = av2_get_sse_plane_available(
    ...
    filt_err = av2_get_sse_plane(sd, &cm->cur_frame->buf, plane);
  }
  ...
  for (int i = 0; i < num_planes; i++) {
    ...
    const int64_t no_deblocking_sse =
        cm->bru.enabled
            ...
            : av2_get_sse_plane(cpi->source, &cm->cur_frame->buf, i);
...
static double try_filter_tip_frame(AV2_COMP *const cpi, int tip_delta) {
  ...
  for (int i = 0; i < num_planes; i++) {
    int64_t cur_sse = av2_get_sse_plane(cpi->source, tip_frame_buf, i);
    filter_sse += cur_sse;
  }
