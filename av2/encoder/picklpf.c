  ...
  for (int r = 0; r < rows; r++) {
    for (int c = 0; c < cols; c++) {
      if (active_mode_map[r * active_map_stride + c] == 2) {
        ...
        const int w = AVMMIN(unit_width, (plane == 0 ? a->y_crop_width : a->uv_crop_width) - x_start);
        const int h = AVMMIN(unit_height, (plane == 0 ? a->y_crop_height : a->uv_crop_height) - y_start);
        if (w > 0 && h > 0) {
