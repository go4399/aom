...
static int64_t avm_get_sse_plane_available(
  ...
  const uint16_t *a_buf = (const uint16_t *)(plane == 0 ? a->y_buffer : (plane == 1 ? a->u_buffer : a->v_buffer));
  const uint16_t *b_buf = (const uint16_t *)(plane == 0 ? b->y_buffer : (plane == 1 ? b->u_buffer : b->v_buffer));
  ...
  for (int r = 0; r < rows; r++) {
    for (int c = 0; c < cols; c++) {
      if (active_mode_map[r * active_map_stride + c] == 2) {
        ...
        const int w = AOMMIN(unit_width, (plane == 0 ? a->y_crop_width : a->uv_crop_width) - x_start);
        const int h = AOMMIN(unit_height, (plane == 0 ? a->y_crop_height : a->uv_crop_height) - y_start);
        if (w > 0 && h > 0) {
