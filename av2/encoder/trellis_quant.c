...
static INLINE void init_tcq_decision(tcq_node_t *decision) {
...
static INLINE void init_tcq_ctx(struct tcq_ctx_t *tcq_ctx) {
...
static INLINE int get_coeff_cost_def(tran_low_t abs_qc, int coeff_ctx,
  ...
  int cost = plane > 0 ? base_cost_uv_ptr[base_ctx][AOMMIN(abs_qc, 3)]
                       : base_cost_ptr[base_ctx][q_i][AOMMIN(abs_qc, 3)];
...
static INLINE int get_diag_ctx(int lf, int blk_pos, int scan_pos, int bwl) {
  int diag_ctx;
  if (lf) {
