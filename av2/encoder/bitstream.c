...
static INLINE void write_delta_qindex(const MACROBLOCKD *xd, int delta_qindex,
  ...
  avm_write_symbol(w, AOMMIN(abs, DELTA_Q_SMALL), ec_ctx->delta_q_cdf,
                   DELTA_Q_PROBS + 1);
...
static INLINE void delta_encode_palette_colors(const int *colors, int num,
  ...
  int bits = AOMMAX(avm_ceil_log2(max_delta + 1 - min_val), min_bits);
  ...
  for (int i = 0; i < num - 1; ++i) {
    ...
    range -= deltas[i];
    bits = AOMMIN(bits, avm_ceil_log2(range));
  }
}
...
int av2_get_index_from_bitdepth(int bit_depth) {
  ...
  switch (bit_depth) {
    case AOM_BITS_10: bitdepth_lut_idx = AVM_BITDEPTH_0; break;
    case AOM_BITS_8: bitdepth_lut_idx = AVM_BITDEPTH_1; break;
    case AOM_BITS_12: bitdepth_lut_idx = AVM_BITDEPTH_2; break;
    default: break;
  }
...
static size_t write_scan_type_metadata(AV2_COMP *const cpi, uint8_t *dst,
  ...
  aom_metadata_t *metadata =
      avm_img_metadata_alloc(OBU_METADATA_TYPE_SCAN_TYPE, payload,
                             avm_wb_bytes_written(&wb), AOM_MIF_ANY_FRAME);
  if (!metadata) {
...
static size_t av2_write_frame_hash_metadata(
  ...
  if (grain_params) {
    ...
    aom_image_t *grain_img = avm_img_alloc(NULL, img.fmt, w_even, h_even, 32);
    if (!grain_img) {
      aom_internal_error(&cpi->common.error, AOM_CODEC_MEM_ERROR,
