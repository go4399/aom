...
static INLINE void accumulate_counters_enc_workers(AV2_COMP *cpi,
                                                   int num_workers) {
  for (int i = num_workers - 1; i >= 0; i--) {
...
static INLINE void prepare_enc_workers(AV2_COMP *cpi, AVxWorkerHook hook,
                                       int num_workers) {
  MultiThreadInfo *const mt_info = &cpi->mt_info;
  for (int i = num_workers - 1; i >= 0; i--) {
    AVxWorker *const worker = &mt_info->workers[i];
...
static INLINE void fp_prepare_enc_workers(AV2_COMP *cpi, AVxWorkerHook hook,
                                          int num_workers) {
  MultiThreadInfo *const mt_info = &cpi->mt_info;
  for (int i = num_workers - 1; i >= 0; i--) {
    AVxWorker *const worker = &mt_info->workers[i];
...
static INLINE int compute_num_enc_row_mt_workers(AV2_COMMON *const cm,
                                                 int max_threads) {
  TileInfo tile_info;
  const int tile_cols = cm->tiles.cols;
  const int tile_rows = cm->tiles.rows;
...
static INLINE int compute_num_enc_tile_mt_workers(AV2_COMMON *const cm,
                                                  int max_threads) {
  const int tile_cols = cm->tiles.cols;
  const int tile_rows = cm->tiles.rows;
...
// stage
static INLINE void compute_max_sb_rows_cols(AV2_COMP *cpi, int *max_sb_rows,
                                            int *max_sb_cols) {
  AV2_COMMON *const cm = &cpi->common;
  const int tile_cols = cm->tiles.cols;
  const int tile_rows = cm->tiles.rows;
...
// stage
static INLINE int fp_compute_max_mb_rows(const AV2_COMMON *const cm,
...
// data.
static INLINE void prepare_tpl_workers(AV2_COMP *cpi, AVxWorkerHook hook,
                                       int num_workers) {
...
static INLINE int compute_num_tpl_workers(AV2_COMP *cpi) {
...
static INLINE int get_next_gm_job(AV2_COMP *cpi, int *frame_idx, int cur_dir) {
...
static INLINE void switch_direction(AV2_COMP *cpi, int *frame_idx,
                                    int *cur_dir) {
...
static INLINE void init_gm_thread_data(const GlobalMotionInfo *gm_info,
...
static INLINE void prepare_gm_workers(AV2_COMP *cpi, AVxWorkerHook hook,
                                      int num_workers) {
  MultiThreadInfo *mt_info = &cpi->mt_info;
  for (int i = num_workers - 1; i >= 0; i--) {
    AVxWorker *worker = &mt_info->workers[i];
...
static INLINE int compute_gm_workers(const AV2_COMP *cpi) {
  int total_refs =
...
// multi-threading.
static INLINE void gm_alloc(AV2_COMP *cpi, int num_workers) {
  AV2_COMMON *cm = &cpi->common;
