...
static void update_drl_index_stats(int max_drl_bits, const int16_t mode_ctx,
  ...
  for (int ref = 0; ref < 1 + has_second_drl(mbmi); ++ref) {
    for (int idx = 0; idx < max_drl_bits; ++idx) {
      aom_cdf_prob *drl_cdf = av2_get_drl_cdf(mbmi, fc, mode_ctx, idx);
...
static void update_warp_delta_stats(const AV2_COMMON *cm,
  ...
  if (mbmi->max_num_warp_candidates > 1) {
    ...
    for (int bit_idx = 0; bit_idx < max_idx_bits; ++bit_idx) {
      aom_cdf_prob *warp_ref_idx_cdf = av2_get_warp_ref_idx_cdf(fc, bit_idx);
...
static INLINE void update_best_level_banks(LevelBanksRDO *level_banks,
...
static INLINE void restore_level_banks(MACROBLOCKD *xd,
...
static INLINE PARTITION_TYPE get_forced_partition_type(
...
static INLINE void init_allowed_partitions(
...
static INLINE bool is_part_pruned_by_forced_partition(
...
static INLINE void set_part_none_allowed_flag(
...
static INLINE bool node_uses_horz(const PC_TREE *pc_tree) {
  assert(pc_tree);
...
static INLINE bool node_uses_vert(const PC_TREE *pc_tree) {
  assert(pc_tree);
 ...
 * */
static INLINE void prune_part_3_with_partition_boundary(
 ...
 * partition boundaries, then they are pruned from the search.
 */
static INLINE void prune_part_4_with_partition_boundary(
...
static INLINE void prune_ext_partitions_3way(
...
static INLINE void prune_ext_partitions_4way(
    AV2_COMP *const cpi, PC_TREE *pc_tree,
    PartitionSearchState *part_search_state, bool *partition_boundaries) {
  const AV2_COMMON *const cm = &cpi->common;
  const PARTITION_SPEED_FEATURES *part_sf = &cpi->sf.part_sf;
...
static INLINE int get_partition_depth(const PC_TREE *pc_tree, int curr_depth) {
...
static INLINE bool try_none_after_rect(const MACROBLOCKD *xd,
 ...
 */
static INLINE void prune_none_with_rect_results(
