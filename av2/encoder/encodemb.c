...
static INLINE int16_t avg_wxh_block_c(const int16_t *diff,
...
static INLINE void avg_wxh_block_horiz_c(const int16_t *diff,
                                         ptrdiff_t diff_stride, int w, int h,
                                         int16_t *out) {
  for (int row = 0; row < h; ++row) {
    int32_t sum = 0;
...
static INLINE void avg_wxh_block_vert_c(const int16_t *diff,
                                        ptrdiff_t diff_stride, int w, int h,
                                        int16_t *out) {
...
void av2_xform_quant(const AV2_COMMON *cm, MACROBLOCK *x, int plane, int block,
  ...
  if (is_cctx_allowed(cm, xd)) {
    ...
    if (plane == AOM_PLANE_U) {
      av2_xform(x, AOM_PLANE_V, block, blk_row, blk_col, plane_bsize,
                txfm_param, 0, NULL);
...
void av2_encode_block_intra_joint_uv(int block, int blk_row, int blk_col,
  ...
  av2_predict_intra_block_facade(cm, xd, AOM_PLANE_U, blk_col, blk_row,
                                 tx_size);
  av2_predict_intra_block_facade(cm, xd, AOM_PLANE_V, blk_col, blk_row,
                                 tx_size);
  ...
  av2_subtract_txb(x, AOM_PLANE_U, plane_bsize, blk_col, blk_row, tx_size,
                   cm->width, cm->height, get_primary_tx_type(tx_type));
  av2_subtract_txb(x, AOM_PLANE_V, plane_bsize, blk_col, blk_row, tx_size,
                   cm->width, cm->height, get_primary_tx_type(tx_type));
  ...
  av2_setup_xform(cm, x, AOM_PLANE_U, tx_size, tx_type, cctx_type, &txfm_param);
  av2_setup_quant(tx_size, use_trellis, quant_idx,
                  cpi->oxcf.q_cfg.quant_b_adapt, &quant_param);
...
static void encode_block_intra_and_set_context_joint_uv(
  ...
  av2_set_txb_context(x, AOM_PLANE_U, block, tx_size, au, lu);
  av2_set_txb_context(x, AOM_PLANE_V, block, tx_size, av, lv);
}
