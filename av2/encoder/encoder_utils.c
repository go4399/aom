...
const int
    av2_default_tx_type_probs[FRAME_UPDATE_TYPES][TX_SIZES_ALL_AV2][TX_TYPES] = {
    ...
    if (cm->current_frame.frame_type == KEY_FRAME) {
      ...
      if (cm->ci_params_per_layer[cm->mlayer_id]
          ...
          cm->ci_params_per_layer[cm->mlayer_id]
                  .color_info.matrix_coefficients == AOM_CICP_MC_SMPTE_YCGCO ||
          cm->ci_params_per_layer[cm->mlayer_id]
                  .color_info.matrix_coefficients == AOM_CICP_MC_YCGCO_RE ||
          cm->ci_params_per_layer[cm->mlayer_id]
                  .color_info.matrix_coefficients == AOM_CICP_MC_YCGCO_RO)
      ...
      if (ref->y_crop_width != cm->width || ref->y_crop_height != cm->height) {
        ...
        if (force_scaling || new_fb->buf.y_crop_width != cm->width ||
          ...
          if (aom_realloc_frame_buffer(
                  ...
                  (cm->seq_params.bit_depth > AOM_BITS_8), AOM_BORDER_IN_PIXELS,
...
static void screen_content_tools_determination(
  ...
  aom_calc_highbd_psnr(cpi->source, b, &psnr[pass], bit_depth, in_bit_depth,
...
void av2_set_mb_ssim_rdmult_scaling(AV2_COMP *cpi) {
  ...
  uint16_t *y_buf = (uint16_t *)cpi->source->y_buffer;
    ...
    for (int col = 0; col < num_cols; ++col) {
      ...
      for (int mi_row = row * num_mi_h;
        ...
        for (int mi_col = col * num_mi_w;
          ...
          buf.buf = y_buf + row_offset_y * y_stride + col_offset_y;
          buf.stride = y_stride;
