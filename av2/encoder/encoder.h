...
static INLINE void avm_wb_write_primitive_quniform(struct aom_write_bit_buffer *wb, uint16_t n, uint16_t v) {
  if (n <= 1) return;
  assert(v < n);
...
static INLINE void wb_write_primitive_subexpfin(struct aom_write_bit_buffer *wb, uint16_t n, uint16_t k, uint16_t v) {
  int i = 0;
  int mk = 0;
  while (1) {
    int b = (i ? k + i - 1 : k);
...
static INLINE void avm_wb_write_primitive_refsubexpfin(struct aom_write_bit_buffer *wb, uint16_t n, uint16_t k, uint16_t ref, uint16_t v) {
  assert(ref < n);
  assert(v < n);
  wb_write_primitive_subexpfin(wb, n, k, recenter_finite_nonneg(n, ref, v));
}

static INLINE void avm_wb_write_signed_primitive_refsubexpfin(struct aom_write_bit_buffer *wb, uint16_t n, uint16_t k, int16_t ref, int16_t v) {
  assert(n > 0);
  const uint16_t offset = n - 1;
  const uint16_t scaled_n = (n << 1) - 1;
  avm_wb_write_primitive_refsubexpfin(wb, scaled_n, k, ref + offset, v + offset);
}

static INLINE int wb_count_primitive_quniform(uint16_t n, uint16_t v) {
  if (n <= 1) return 0;
  assert(v < n);
  const int l = get_msb(n) + 1;
  const int m = (1 << l) - n;
...
static INLINE int wb_count_primitive_subexpfin(uint16_t n, uint16_t k, uint16_t v) {
  int bits = 0;
  int i = 0;
  int mk = 0;
  while (1) {
...
static INLINE int avm_wb_count_primitive_refsubexpfin(uint16_t n, uint16_t k, int16_t ref, int16_t v) {
  assert(ref < n);
  assert(v < n);
...
static INLINE void avm_write_primitive_quniform(aom_writer *w, uint16_t n, uint16_t v) {
  if (n <= 1) return;
...
typedef struct {
  ...
  av2_opfl_refine_type enable_opfl_refine;
...
typedef struct AV2_COMP {
   ...
   * size i.
   */
  aom_variance_fn_ptr_t fn_ptr[ENCODER_BLOCK_SIZES_ALL];
...
// options and speed.
static INLINE void enforce_max_ref_frames(AV2_COMP *cpi,
                                              int *ref_frame_flags) {
...
static INLINE int is_psnr_calc_enabled(const AV2_COMP *cpi) {
