...
// ---------------------------------------------------------------------------------
// C-Fallback Implementations for SAD, 4D SAD, distance-weighted SAD, and masked
// SAD
// ---------------------------------------------------------------------------------
...
static inline unsigned int generic_hbd_sad_avg(
...
static inline unsigned int generic_hbd_dist_wtd_sad_avg(
    const uint8_t *src8, int src_stride, const uint8_t *ref8, int ref_stride,
    const uint8_t *second_pred8, const struct dist_wtd_comp_params *jcp_param,
    int w, int h) {
...
static inline unsigned int generic_hbd_masked_sad(
    const uint8_t *src8, int src_stride, const uint8_t *ref8, int ref_stride,
    const uint8_t *second_pred8, const uint8_t *msk, int msk_stride,
  ...
  for (r = 0; r < h; r++) {
    for (c = 0; c < w; c++) {
      ...
      int pred =
          (ref[c] * mask_val + second_pred[c] * (64 - mask_val) + 32) >> 6;
      sad += abs(src[c] - pred);
    }
    src += src_stride;
    ref += ref_stride;
...
// ---------------------------------------------------------------------------------
// C-Fallback Implementations for non-standard HBD sizes to guarantee build
// success
// ---------------------------------------------------------------------------------

static inline unsigned int generic_hbd_variance(const uint8_t *src8,
                                                int src_stride,
                                                const uint8_t *ref8,
...
static inline unsigned int generic_hbd_masked_variance(
    const uint8_t *src8, int src_stride, const uint8_t *ref8, int ref_stride,
    const uint8_t *second_pred8, const uint8_t *msk, int msk_stride,
    int invert_mask, int w, int h, unsigned int *sse) {
  const uint16_t *src = CONVERT_TO_SHORTPTR(src8);
  const uint16_t *ref = CONVERT_TO_SHORTPTR(ref8);
  const uint16_t *second_pred = CONVERT_TO_SHORTPTR(second_pred8);
  int r, c;
...
#define DEFINE_HBD_COMPATIBILITY_SIZE_RXTX(W, H, BD)                          \
  static inline unsigned int aom_highbd_##BD##_variance##W##x##H(             \
      const uint8_t *src, int src_stride, const uint8_t *ref, int ref_stride, \
      unsigned int *sse) {                                                    \
    return generic_hbd_variance(src, src_stride, ref, ref_stride, W, H, sse); \
  }                                                                           \
  static inline unsigned int aom_highbd_##BD##_sub_pixel_variance##W##x##H(   \
  ...
  aom_highbd_##BD##_sub_pixel_avg_variance##W##x##H(                          \
      const uint8_t *src, int src_stride, int xoffset, int yoffset,           \
      const uint8_t *ref, int ref_stride, unsigned int *sse,                  \
  ...
  static inline unsigned int                                                  \
  aom_highbd_##BD##_dist_wtd_sub_pixel_avg_variance##W##x##H(                 \
      const uint8_t *src, int src_stride, int xoffset, int yoffset,           \
      const uint8_t *ref, int ref_stride, unsigned int *sse,                  \
      const uint8_t *second_pred,                                             \
...
#define DEFINE_HBD_MASKED_COMPATIBILITY_SIZE(W, H, BD)                   \
  static inline unsigned int                                             \
  aom_highbd_##BD##_masked_sub_pixel_variance##W##x##H(                  \
      const uint8_t *src, int src_stride, int xoffset, int yoffset,      \
    ...
    (void)xoffset;                                                       \
    (void)yoffset;                                                       \
    return generic_hbd_masked_variance(src, src_stride, ref, ref_stride, \
                                       second_pred, msk, msk_stride,     \
                                       invert_mask, W, H, sse);          \
  }
