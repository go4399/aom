...
static INLINE void update_wedge_mode_cdf(FRAME_CONTEXT *fc,
...
static INLINE int set_segment_rdmult(const AV2_COMP *const cpi,
                                     MACROBLOCK *const x, int8_t segment_id) {
...
static INLINE void set_max_min_partition_size(SuperBlockEnc *sb_enc,
  ...
  if (use_auto_max_partition(cpi, sb_size, mi_row, mi_col)) {
    ...
    sb_enc->max_partition_size =
        AOMMAX(AOMMIN(av2_predict_max_partition(cpi, x, features),
                      sb_enc->max_partition_size),
               sb_enc->min_partition_size);
  }
}
...
static INLINE void alloc_inter_modes_info_data(AV2_COMMON *const cm,
...
static INLINE void dealloc_inter_modes_info_data(struct macroblock *mb) {
