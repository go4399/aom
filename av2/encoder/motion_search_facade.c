...
static INLINE void build_second_inter_pred(const AV2_COMP *cpi, MACROBLOCK *x,
                                           BLOCK_SIZE bsize, const MV *other_mv,
                                           int ref_idx, uint16_t *second_pred) {
  const AV2_COMMON *const cm = &cpi->common;
  const int pw = block_size_wide[bsize];
  const int ph = block_size_high[bsize];
  MACROBLOCKD *xd = &x->e_mbd;
...
int_mv av2_simple_motion_sse_var(AV2_COMP *cpi, MACROBLOCK *x, int mi_row,
  ...
  *var = cpi->fn_ptr[bsize].vf((const uint8_t *)src, src_stride,
