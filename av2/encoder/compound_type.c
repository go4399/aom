...
static int8_t estimate_wedge_sign(const AV2_COMP *cpi, const MACROBLOCK *x,
  ...
  cpi->fn_ptr[f_index].vf((const uint8_t *)src, src_stride, (const uint8_t *)pred0, stride0, &esq[0][0]);
  cpi->fn_ptr[f_index].vf((const uint8_t *)(src + bh_by2 * src_stride + bw_by2), src_stride,
                          (const uint8_t *)(pred0 + bh_by2 * stride0 + bw_by2), stride0,
                          &esq[0][1]);
  cpi->fn_ptr[f_index].vf((const uint8_t *)src, src_stride, (const uint8_t *)pred1, stride1, &esq[1][0]);
  cpi->fn_ptr[f_index].vf((const uint8_t *)(src + bh_by2 * src_stride + bw_by2), src_stride,
                          (const uint8_t *)(pred1 + bh_by2 * stride1 + bw_by2), stride0,
                          &esq[1][1]);
...
static INLINE void get_inter_predictor_masked_compound_y(
...
static int64_t masked_compound_type_rd(
  ...
  if (cpi->sf.inter_sf.prune_wedge_pred_diff_based &&
    ...
    unsigned int sse;
    (void)cpi->fn_ptr[bsize].vf((const uint8_t *)pred0, stride, (const uint8_t *)pred1, stride, &sse);
