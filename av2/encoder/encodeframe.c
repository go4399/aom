...
unsigned int av2_high_get_sby_perpixel_variance(const AV2_COMP *cpi,
  ...
  var = cpi->fn_ptr[bs].vf((const uint8_t *)ref->buf, ref->stride, (const uint8_t *)high_var_offs[off_index], 0,
                           &sse);
...
static unsigned int get_sby_perpixel_diff_variance(const AV2_COMP *const cpi,
  ...
  var = cpi->fn_ptr[bs].vf((const uint8_t *)ref->buf, ref->stride, (const uint8_t *)last_y, last->y_stride, &sse);
  return ROUND_POWER_OF_TWO(var, num_pels_log2_lookup[bs]);
}
...
void av2_setup_src_planes(MACROBLOCK *x, const YV12_BUFFER_CONFIG *src,
  ...
  for (int i = 0; i < AOMMIN(num_planes, MAX_MB_PLANE); i++) {
    const int is_uv = i > 0;
    setup_pred_plane(&x->plane[i].src, (uint16_t *)src->buffers[i], src->crop_widths[is_uv],
 ...
 */
static INLINE void setup_delta_q(AV2_COMP *const cpi, ThreadData *td,
                                     MACROBLOCK *const x,
...
static INLINE void adjust_rdmult_tpl_model(AV2_COMP *cpi, MACROBLOCK *x,
                                               int mi_row, int mi_col) {
 ...
 */
static INLINE void perform_one_partition_pass(
 ...
 */
static INLINE void perform_two_partition_passes(
    AV2_COMP *cpi, ThreadData *td, TileDataEnc *tile_data, TokenExtra **tp,
    TokenExtra **tp_chroma, const int mi_row, const int mi_col) {
  SIMPLE_MOTION_DATA_TREE *const sms_root = td->sms_root;
  AV2_COMMON *const cm = &cpi->common;
 ...
 * \ingroup partition_search
 */
static INLINE void set_min_none_to_invalid(PARTITION_TREE *part_tree,
 ...
 */
static INLINE void perform_two_pass_partition_search(
    AV2_COMP *cpi, ThreadData *td, TileDataEnc *tile_data, TokenExtra **tp,
    TokenExtra **tp_chroma, const int mi_row, const int mi_col) {
  SIMPLE_MOTION_DATA_TREE *const sms_root = td->sms_root;
  AV2_COMMON *const cm = &cpi->common;
 ...
 */
static INLINE void encode_rd_sb(AV2_COMP *cpi, ThreadData *td,
...
static INLINE void bridge_frame_set_offsets(
...
static INLINE void bridge_frame_predict_inter_block(AV2_COMMON *const cm,
...
static INLINE void bridge_frame_decode_partition(
...
static INLINE void bridge_frame_decode_partition_sb(
    AV2_COMP *cpi, ThreadData *const td, const TileInfo *const tile, int mi_row,
 ...
 */
static INLINE void encode_sb_row(AV2_COMP *cpi, ThreadData *td,
...
static INLINE void init_encode_frame_mb_context(AV2_COMP *cpi) {
  AV2_COMMON *const cm = &cpi->common;
  const int num_planes = av2_num_planes(cm);
 ...
 */
static INLINE void encode_tiles(AV2_COMP *cpi) {
  AV2_COMMON *const cm = &cpi->common;
  const int tile_cols = cm->tiles.cols;
  const int tile_rows = cm->tiles.rows;
  int tile_col, tile_row;
...
static INLINE int could_tip_mode_be_selected(AV2_COMP *const cpi) {
  const AV2_COMMON *const cm = &cpi->common;
...
static INLINE void av2_enc_setup_tip_frame(AV2_COMP *cpi) {
  AV2_COMMON *const cm = &cpi->common;
 ...
 * \ingroup high_level_algo
 */
static INLINE void encode_frame_internal(AV2_COMP *cpi) {
  ThreadData *const td = &cpi->td;
  MACROBLOCK *const x = &td->mb;
  AV2_COMMON *const cm = &cpi->common;
