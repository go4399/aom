...
static INLINE void generate_hog_hbd(const uint16_t *src, int stride, int rows,
                                    int cols, float *hist) {
  float total = 0.1f;
  src += stride;
 ...
 * \callergraph
 */
static INLINE int intra_mode_info_cost_y(const AV2_COMP *cpi,
                                         const MACROBLOCK *x,
  ...
  if (try_palette && mbmi->mode == DC_PRED) {
    total_rate += mode_costs->palette_y_mode_cost[use_palette];
    if (use_palette) {
 ...
 */
static INLINE int model_intra_yrd_and_prune(const AV2_COMP *const cpi,
