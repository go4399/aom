...
static INLINE void init_flexmv_sf(FLEXMV_PRECISION_SPEED_FEATURES *flexmv_sf) {
...
static INLINE void set_erp_speed_features_framesize_dependent(AV2_COMP *cpi) {
...
static INLINE void set_erp_speed_features(AV2_COMP *cpi) {
  SPEED_FEATURES *const sf = &cpi->sf;
  const AV2_COMMON *const cm = &cpi->common;
  const GF_GROUP *const gf_group = &cpi->gf_group;
  const int boosted = frame_is_boosted(cpi);
...
static INLINE void set_erp_speed_features_qindex_dependent(AV2_COMP *cpi) {
  SPEED_FEATURES *const sf = &cpi->sf;
  const AV2_COMMON *const cm = &cpi->common;
  const int is_1080p_or_larger = AOMMIN(cm->width, cm->height) >= 1080;
  const unsigned int erp_pruning_level = cpi->oxcf.part_cfg.erp_pruning_level;
