...
static const sse_part_extractor_type sse_part_extractors[NUM_EXTRACTORS] = {
  av2_highbd_get_y_sse_part,
  av2_highbd_get_u_sse_part,
  av2_highbd_get_v_sse_part,
};
...
static INLINE void reset_all_banks(RestSearchCtxt *rsc) {
...
static INLINE void rsc_on_tile(void *priv, int idx_base, int tile_row,
                               int tile_col) {
...
static INLINE void reset_rsc(RestSearchCtxt *rsc) {
  rsc->sse = 0;
  rsc->bits = 0;
  av2_vector_clear(rsc->unit_stack);
  av2_vector_clear(rsc->unit_indices);
}

static INLINE void init_rsc(const YV12_BUFFER_CONFIG *src, const AV2_COMMON *cm,
                            const MACROBLOCK *x,
                            const LOOP_FILTER_SPEED_FEATURES *lpf_sf, int plane,
                            RestUnitSearchInfo *rusi, Vector *unit_stack,
                            Vector *unit_indices, YV12_BUFFER_CONFIG *dst,
                            RestSearchCtxt *rsc) {
  rsc->src = src;
  rsc->dst = dst;
  rsc->cm = cm;
...
static INLINE void search_pc_wiener_visitor(const RestorationTileLimits *limits,
                                            const AV2PixelRect *tile_rect,
                                            int rest_unit_idx,
...
static int64_t calc_finer_tile_search_error(const RestSearchCtxt *rsc,
  ...
  if (limits != NULL) {
    ...
    int n = 0;
    int idx = *(int *)av2_vector_const_get(current_unit_indices, n);
    VECTOR_FOR_EACH(current_unit_stack, listed_unit) {
      ...
      if (old_unit->rest_unit_idx == idx && !rsc->rusi[idx].bru_unit_skipped) {
        ...
        idx = *(int *)av2_vector_const_get(current_unit_indices, n);
      }
    }
  }
  return err;
...
static int64_t reset_unit_stack_dst_buffers(const RestSearchCtxt *rsc,
  ...
  if (limits != NULL) {
    ...
    Vector *current_unit_indices = rsc->unit_indices;
    const int last_idx =
        ((RstUnitSnapshot *)av2_vector_back(current_unit_stack))->rest_unit_idx;
...
static INLINE void search_norestore_visitor(const RestorationTileLimits *limits,
                                            const AV2PixelRect *tile_rect,
                                            int rest_unit_idx,
                                            int rest_unit_idx_seq, void *priv,
                                            RestorationLineBuffers *rlbs) {
...
void populate_current_unit_indices(
  ...
  VECTOR_FOR_EACH(current_unit_stack, listed_unit) {
    ...
    av2_vector_push_back(current_unit_indices, &index);
  }
}
...
static void gather_stats_wienerns(const RestorationTileLimits *limits,
  ...
  av2_vector_push_back(rsc->wienerns_stats, &unit_stats);
  return;
}
...
static void search_wienerns_visitor(const RestorationTileLimits *limits,
  ...
  const RstUnitStats *unit_stats = (const RstUnitStats *)av2_vector_const_get(
  ...
  if (av2_vector_is_empty(current_unit_stack)) {
    if (rtype == RESTORE_WIENER_NONSEP)
      av2_add_to_wienerns_bank(&rsc->wienerns_bank, &rusi->wienerns_info,
                               ALL_WIENERNS_CLASSES);
    av2_vector_push_back(current_unit_stack, &unit_snapshot);
    if (rusi->best_rtype[RESTORE_WIENER_NONSEP - 1] == RESTORE_WIENER_NONSEP)
      rsc->num_wiener_nonsep++;
    return;
  }
  ...
  for (int c_id = 0; c_id < num_classes; ++c_id) {
    ...
    for (int bank_ref_cand = 0;
      ...
      // filter.
      Vector *current_unit_indices = rsc->unit_indices;
      av2_vector_clear(current_unit_indices);
    ...
    RstUnitSnapshot *last_unit = av2_vector_back(current_unit_stack);
...
static INLINE void bru_set_sru_skip(RestSearchCtxt *rsc, int rrow0, int rrow1,
                                    int rcol0, int rcol1) {
...
static void gather_stats_rest_type(RestSearchCtxt *rsc, RestorationType rtype) {
  ...
  if (rtype == RESTORE_WIENER_NONSEP) av2_vector_clear(rsc->wienerns_stats);
...
static void weighted_sum_all_stats(const RestSearchCtxt *rsc,
  ...
  const RstUnitStats *sample_stat =
      av2_vector_begin(rsc->wienerns_stats).pointer;
...
int count_match_indices_bits(int plane, int num_classes, int num_ref_frames,
  ...
  for (int c_id = 0; c_id < num_classes; ++c_id) {
    ...
    if (n > 1) {
      total_bits += av2_wb_count_primitive_refsubexpfin(
...
static INLINE void initialize_stat_weights(RestSearchCtxt *rsc) {
  VECTOR_FOR_EACH(rsc->wienerns_stats, unit_stats) {
    RstUnitStats *unit_stats_ptr = (RstUnitStats *)(unit_stats.pointer);
    unit_stats_ptr->weight = RU_ON_WEIGHT;
  }
...
static double optimize_frame_filters_with_rounding(
  ...
  VECTOR_FOR_EACH(rsc->wienerns_stats, unit_stats) {
    ...
    av2_vector_push_back(current_unit_stack, &unit_snapshot);
    av2_vector_push_back(current_unit_indices, &stat_slot);
  }
  if (cnt == 0) return -1;
...
void av2_pick_filter_restoration(const YV12_BUFFER_CONFIG *src, AV2_COMP *cpi) {
  ...
  Vector wienerns_stats;
  av2_vector_setup(&wienerns_stats,
                   1,                             // resizable capacity
  ...
  for (int plane = plane_start; plane <= plane_end; ++plane) {
    ...
    for (int unit_size = min_unit_size; unit_size <= max_unit_size;
      ...
      av2_vector_clear(&wienerns_stats);
  ...
  av2_vector_destroy(&wienerns_stats);
  av2_vector_destroy(&unit_stack);
  av2_vector_destroy(&unit_indices);
}
