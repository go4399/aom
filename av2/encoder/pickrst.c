...
static int64_t try_restoration_unit(const RestSearchCtxt *rsc,
  ...
  av2_loop_restoration_filter_unit(
      ...
      fts->strides[is_uv], rsc->dst->buffers_u16[plane], rsc->dst->strides[is_uv],
...
static INLINE void search_pc_wiener_visitor(
...
static INLINE void search_norestore_visitor(
    const RestorationTileLimits *limits, const AV2PixelRect *tile_rect,
    int rest_unit_idx, int rest_unit_idx_seq, void *priv,
    RestorationLineBuffers *rlbs) {
  (void)tile_rect;
...
static INLINE void bru_set_sru_skip(RestSearchCtxt *rsc, int rrow0,
