...
void av2_build_quantizer(aom_bit_depth_t bit_depth, int y_dc_delta_q,
...
static void set_qm_params(AV2_COMMON *const cm,
                          CommonQuantParams *const quant_params,
                          int min_qmlevel, int max_qmlevel) {
  int qm_y = av2_get_qmlevel(quant_params->base_qindex, min_qmlevel,
                             max_qmlevel, cm->seq_params.bit_depth);
  int qm_u =
      av2_get_qmlevel(quant_params->base_qindex + quant_params->u_ac_delta_q,
                      min_qmlevel, max_qmlevel, cm->seq_params.bit_depth);

  int qm_v = qm_u;
  if (cm->seq_params.separate_uv_delta_q) {
    qm_v =
        av2_get_qmlevel(quant_params->base_qindex + quant_params->v_ac_delta_q,
                        min_qmlevel, max_qmlevel, cm->seq_params.bit_depth);
  }
...
int av2_quantizer_to_qindex(int quantizer, aom_bit_depth_t bit_depth) {
  assert(quantizer <= 63);
  switch (bit_depth) {
...
int av2_qindex_to_quantizer(int qindex, aom_bit_depth_t bit_depth) {
  int quantizer;
...
#include "av2/encoder/av2_quantize_helper.c"
