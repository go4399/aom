...
int av2_band_detection_init(Av2BandDetectInfo *const dbi, const int frame_width,
  ...
  dbi->frame = aom_malloc(sizeof(*dbi->frame) * frame_height * dbi->stride);
  ...
  dbi->buffers.filter_mode_buffer =
      aom_malloc(3 * frame_width * sizeof(uint16_t));
  ...
  dbi->mask = aom_malloc(sizeof(*dbi->mask) * frame_height * dbi->stride);
  dbi->buffers.c_values =
      aom_malloc(sizeof(float) * frame_height * dbi->stride);
