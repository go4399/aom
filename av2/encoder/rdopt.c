...
static INLINE void inter_modes_info_sort(
...
static int64_t get_sse(const AV2_COMP *cpi, const MACROBLOCK *x,
  ...
  for (int plane = 0; plane < num_planes; ++plane) {
    ...
    cpi->fn_ptr[bs].vf((const uint8_t *)p->src.buf, p->src.stride, (const uint8_t *)pd->dst.buf, pd->dst.stride,
                       &sse);
    total_sse += sse;
...
static INLINE void default_skip_mask(mode_skip_mask_t *mask,
                                         REF_SET ref_set) {
