...
static int64_t get_sse(const AV2_COMP *cpi, const MACROBLOCK *x,
  ...
  for (int plane = 0; plane < num_planes; ++plane) {
    ...
    cpi->fn_ptr[bs].vf((const uint8_t *)p->src.buf, p->src.stride,
                       (const uint8_t *)pd->dst.buf, pd->dst.stride, &sse);
    total_sse += sse;
  ...
  if (seg_ref_active) {
    ...
    for (int i = 0; i < n_refs; i++) {
      ...
      for (int j = 0; j <= AOMMIN(i, n_refs - 2); j++) {
        ...
        aom_cdf_prob ctx = av2_get_ref_pred_context(xd, j, n_refs);
        const int bit = i == j;
...
static INLINE void store_coding_context(MACROBLOCK *x, PICK_MODE_CONTEXT *ctx,
                                        int64_t comp_pred_diff[REFERENCE_MODES],
                                        int skippable, const AV2_COMMON *cm) {
...
void av2_rd_pick_intra_mode_sb(const struct AV2_COMP *cpi, ThreadData *td,
  ...
  if (intra_yrd < best_rd) {
    ...
    if (num_planes > 1 && xd->tree_type != LUMA_PART) {
      ...
      const TX_SIZE max_uv_tx_size = av2_get_tx_size(AOM_PLANE_U, xd);
...
static INLINE void disable_inter_references_except_top(
    bool ref_combo[MAX_COMPOUND_REF_INDEX][MAX_COMPOUND_REF_INDEX + 1]) {
...
static INLINE int prune_ref_frame(const AV2_COMP *cpi, const MACROBLOCK *x,
                                  const MV_REFERENCE_FRAME ref_frame) {
  const AV2_COMMON *const cm = &cpi->common;
...
static INLINE void collect_single_states(const AV2_COMMON *const cm,
                                         MACROBLOCK *x,
                                         InterModeSearchState *search_state,
                                         const MB_MODE_INFO *const mbmi) {
  const FeatureFlags *const features = &cm->features;
...
static INLINE void analyze_single_states(const AV2_COMP *cpi,
                                         InterModeSearchState *search_state) {
...
static INLINE void find_top_ref(int64_t *ref_frame_rd) {
