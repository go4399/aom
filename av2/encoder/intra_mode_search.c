...
static INLINE int get_chroma_idx_for_reuse_uvrd(MB_MODE_INFO *mbmi) {
...
static INLINE void store_uv_mode_rd_info(ModeRDInfoUV *mode_rd_info_uv,
                                         RD_STATS *tokenonly_rd_stats,
...
static INLINE void fetch_uv_mode_rd_info(ModeRDInfoUV *mode_rd_info_uv,
                                         RD_STATS *tokenonly_rd_stats,
                                         const int chroma_idx) {
  ...
  if (num_planes > 1) {
    {
      ...
      TX_SIZE uv_tx = av2_get_tx_size(AOM_PLANE_U, xd);
 ...
 */
static INLINE int intra_block_yrd(const AV2_COMP *const cpi, MACROBLOCK *x,
  ...
  if (this_rd < *best_rd) {
    ...
    *skippable = rd_stats.skip_txfm;
    av2_copy_array(ctx->blk_skip[AOM_PLANE_Y],
...
int64_t av2_handle_intra_mode(IntraModeSearchState *intra_search_state,
  ...
  if (num_planes > 1) {
    ...
    const TX_SIZE uv_tx = av2_get_tx_size(AOM_PLANE_U, xd);
...
int64_t av2_rd_pick_intra_sby_mode(const AV2_COMP *const cpi, ThreadData *td,
  ...
  if (cpi->sf.winner_mode_sf.multi_winner_mode_type) {
    ...
    av2_get_block_dimensions(bsize, AOM_PLANE_Y, xd, &block_width,
