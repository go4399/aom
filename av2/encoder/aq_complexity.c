...
static int get_aq_c_strength(int q_index, aom_bit_depth_t bit_depth) {
  ...
  const int base_quant =
      av2_ac_quant_QTX(q_index, 0, 0, bit_depth) / (4 << QUANT_TABLE_BITS);
  return (base_quant > 10) + (base_quant > 25);
}

static bool is_frame_aq_enabled(const AV2_COMP *const cpi) {
...
static bool is_sb_aq_enabled(const AV2_COMP *const cpi) {
  return cpi->rc.sb64_target_rate >= 256;
}

void av2_setup_in_frame_q_adj(AV2_COMP *cpi) {
  ...
  if (is_frame_aq_enabled(cpi)) {
    ...
    av2_disable_segfeature(seg, DEFAULT_AQ2_SEG, SEG_LVL_ALT_Q);
    ...
    for (segment = 0; segment < AQ_C_SEGMENTS; ++segment) {
      ...
      if ((base_qindex + qindex_delta) > 0) {
        av2_enable_segfeature(seg, segment, SEG_LVL_ALT_Q);
        av2_set_segdata(seg, segment, SEG_LVL_ALT_Q, qindex_delta);
      }
    }
  }
}
...
void av2_caq_select_segment(const AV2_COMP *cpi, MACROBLOCK *mb, BLOCK_SIZE bs,
  ...
  // It is converted to bits << AV2_PROB_COST_SHIFT units.
  ...
  av2_setup_src_planes(mb, cpi->source, mi_row, mi_col, num_planes,
                       &mb->e_mbd.mi[0]->chroma_ref_info);
  logvar = av2_log_block_var(cpi, mb, bs
