...
static INLINE void tpl_subtract_block(
...
static INLINE int64_t tpl_get_satd_cost(const MACROBLOCK *x,
    ...
    if (cpi->sf.tpl_sf.prune_starting_mv) {
      ...
      for (idx = 0; idx < refmv_count; ++idx) {
        ...
        center_mvs[idx].sad = (int)cpi->fn_ptr[bsize].sdf(
            (const uint8_t *)src_mb_buffer, src_stride, (const uint8_t *)&ref_mb[mv.row * ref_stride + mv.col],
            ref_stride);
      }
