...
static INLINE void tpl_fwd_txfm(const int16_t *src_diff, int bw,
...
static INLINE void tpl_subtract_block(const MACROBLOCKD *xd, int rows, int cols,
...
static INLINE int64_t tpl_get_satd_cost(const MACROBLOCK *x, int16_t *src_diff,
                                        int diff_stride, const uint16_t *src,
...
static INLINE void mode_estimation(AV2_COMP *cpi, MACROBLOCK *x, int mi_row,
    ...
    if (cpi->sf.tpl_sf.prune_starting_mv) {
      ...
      for (idx = 0; idx < refmv_count; ++idx) {
        ...
        center_mvs[idx].sad = (int)cpi->fn_ptr[bsize].sdf(
            (const uint8_t *)src_mb_buffer, src_stride,
            (const uint8_t *)&ref_mb[mv.row * ref_stride + mv.col], ref_stride);
      }
...
static INLINE void init_mc_flow_dispenser(AV2_COMP *cpi, int frame_idx,
                                          int pframe_qindex) {
...
static INLINE void mc_flow_dispenser(AV2_COMP *cpi) {
  AV2_COMMON *cm = &cpi->common;
  const CommonModeInfoParams *const mi_params = &cm->mi_params;
  ThreadData *td = &cpi->td;
  MACROBLOCK *x = &td->mb;
