...
static INLINE void avm_wb_write_uleb(struct aom_write_bit_buffer *wb, uint32_t value) {
  do {
...
static INLINE void avm_wb_write_rice_golomb(struct aom_write_bit_buffer *wb, uint32_t data, int k) {
  assert(k <= 26);
...
static INLINE void avm_wb_write_svlc(struct aom_write_bit_buffer *wb, int32_t value) {
  uint32_t uvlc;
  if (value > 0) {
...
static INLINE void avm_wb_write_primitive_ref_quniform(struct aom_write_bit_buffer *wb, uint16_t n, uint16_t r, uint16_t v) {
  if (v == r) {
    aom_wb_write_bit(wb, 0);
  } else {
    aom_wb_write_bit(wb, 1);
...
// Standard AV2 typedef forward-declarations to align pointer parameters perfectly
...
uint32_t av2_write_layer_configuration_record_obu(AV2_COMP *cpi,
                                                  int xlayer_id,
                                                  uint8_t *const dst);
uint32_t av2_write_atlas_segment_info_obu(AV2_COMP *cpi,
                                          uint8_t *const dst);
