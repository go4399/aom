...
static INLINE int get_vq_col_mvd_rate(nmv_context *mvctx,
...
static INLINE int get_truncated_unary_rate(nmv_context *mvctx,
...
static INLINE int get_vq_mvd_rate(nmv_context *mvctx, const MV mv_diff,
...
static INLINE void keep_vq_one_mv_stat(MV_STATS *mv_stats, const MV *ref_mv,
                                       const MV *cur_mv, const AV2_COMP *cpi,
...
static INLINE void collect_mv_stats_b(MV_STATS *mv_stats, const AV2_COMP *cpi,
  ...
  uint16_t *source_buf = CONVERT_TO_SHORTPTR(cpi->source->y_buffer) + px_row * y_stride + px_col;
...
// Split block
static INLINE void collect_mv_stats_sb(MV_STATS *mv_stats, const AV2_COMP *cpi,
...
static INLINE int get_smart_mv_prec(AV2_COMP *cpi, const MV_STATS *mv_stats,
                                    int current_q) {
  const AV2_COMMON *cm = &cpi->common;
