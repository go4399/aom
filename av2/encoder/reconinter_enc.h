...
struct AV1Common;
void aom_highbd_upsampled_pred(
...
static inline void avm_highbd_upsampled_pred(
  ...
  (void)is_scaled;
  aom_highbd_upsampled_pred(xd, (const struct AV1Common *)cm, mi_row, mi_col, mv,
                            (uint8_t *)comp_pred, width, height, subpel_x_q3,
                            subpel_y_q3, (const uint8_t *)ref, ref_stride,
                            bd, subpel_search);
}
...
static inline void avm_highbd_comp_avg_upsampled_pred(
  ...
  aom_highbd_comp_avg_upsampled_pred(
      xd, (const struct AV1Common *)cm, mi_row, mi_col, mv,
      (uint8_t *)comp_pred, (const uint8_t *)pred, width, height,
      subpel_x_q3, subpel_y_q3, (const uint8_t *)ref, ref_stride,
      bd, subpel_search);
}
...
static inline void avm_highbd_comp_mask_upsampled_pred(
  ...
  aom_highbd_comp_mask_upsampled_pred(
      ...
      (uint8_t *)comp_pred, (const uint8_t *)pred, width, height,
      subpel_x_q3, subpel_y_q3, (const uint8_t *)ref, ref_stride,
      mask, mask_stride, invert_mask, bd, subpel_search);
}
...
static inline void avm_highbd_comp_mask_pred(
    uint16_t *dst, const uint16_t *pred, int width, int height,
    const uint16_t *ref, int ref_stride, const uint8_t *mask, int mask_stride,
    int invert_mask) {
...
static inline void avm_highbd_comp_avg_pred(uint16_t *dst,
