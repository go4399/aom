...
static const int av2_ext_tx_set_idx_to_type[2][AOMMAX(EXT_TX_SETS_INTRA,
                                                      EXT_TX_SETS_INTER)] = {
  {
      // Intra
      EXT_TX_SET_DCTONLY,
...
static int compute_rd_thresh_factor(int qindex, int base_y_dc_delta_q,
  ...
  switch (bit_depth) {
    case AOM_BITS_8:
      q = av2_dc_quant_QTX(qindex, 0, base_y_dc_delta_q, AOM_BITS_8) / 4.0;
      break;
    case AOM_BITS_10:
      q = av2_dc_quant_QTX(qindex, 0, base_y_dc_delta_q, AOM_BITS_10) / 16.0;
      break;
    case AOM_BITS_12:
      q = av2_dc_quant_QTX(qindex, 0, base_y_dc_delta_q, AOM_BITS_12) / 64.0;
      break;
    default:
      assert(0 && "bit_depth should be AOM_BITS_8, AOM_BITS_10 or AOM_BITS_12");
      return -1;
...
void av2_mv_pred(const AV2_COMP *cpi, MACROBLOCK *x, uint16_t *ref_y_buffer,
  ...
  for (int i = 0; i < num_mv_refs; ++i) {
    ...
    const int this_sad = cpi->fn_ptr[block_size].sdf(
        (const uint8_t *)src_y_ptr, x->plane[0].src.stride, (const uint8_t *)ref_y_ptr, ref_y_stride);
