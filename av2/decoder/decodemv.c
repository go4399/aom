...
static int read_cwp_idx(MACROBLOCKD *xd, aom_reader *r, const AV2_COMMON *cm,
                        MB_MODE_INFO *const mbmi) {
  int8_t cwp_idx = 0;
  int bit_cnt = 0;
  const int ctx = 0;
...
static INLINE void read_single_ref(MACROBLOCKD *const xd,
...
static INLINE void read_compound_ref(const MACROBLOCKD *xd,
                                     MV_REFERENCE_FRAME ref_frame[2],
                                     const RefFramesInfo *const ref_frames_info,
                                     aom_reader *r) {
  const int n_refs = ref_frames_info->num_total_refs;
