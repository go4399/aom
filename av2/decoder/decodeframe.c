...
static INLINE void decode_reconstruct_tx(
    AV2_COMMON *cm, ThreadData *const td, aom_reader *r,
...
static INLINE void decode_partition(
...
static INLINE void setup_gdf(AV2_COMMON *cm,
                                 struct aom_read_bit_buffer *rb) {
...
static INLINE void setup_cdef(AV2_COMMON *cm,
                                  struct aom_read_bit_buffer *rb) {
  const int num_planes = av2_num_planes(cm);
...
static INLINE void setup_ccso(AV2_COMMON *cm,
                                  struct aom_read_bit_buffer *rb) {
  if (cm->bru.frame_inactive_flag || cm->bridge_frame_info.is_bridge_frame ||
      cm->features.tip_frame_mode == TIP_FRAME_AS_OUTPUT) {
...
static INLINE void decoder_alloc_tile_data(AV2Decoder *pbi,
                                               const int n_tiles) {
  AV2_COMMON *const cm = &pbi->common;
...
static INLINE void alloc_dec_jobs(AV2DecTileMT *tile_mt_info,
...
static INLINE void reset_dec_workers(AV2Decoder *pbi,
...
static INLINE void launch_dec_workers(AV2Decoder *pbi,
...
static INLINE void tile_mt_queue(AV2Decoder *pbi, int tile_cols,
