...
int av2_check_trailing_bits(AV2Decoder *pbi, struct aom_read_bit_buffer *rb) {
...
static INLINE void set_planes_to_neutral_grey(
  ...
  for (int plane = only_chroma; plane < MAX_MB_PLANE; plane++) {
    ...
    uint16_t *const base = (uint16_t *)buf->buffers[plane];
...
static INLINE void loop_restoration_read_sb_coeffs(AV2_COMMON *cm,
                                                   MACROBLOCKD *xd,
                                                   aom_reader *const r,
...
static REFERENCE_MODE read_frame_reference_mode(
    const AV2_COMMON *cm, struct aom_read_bit_buffer *rb) {
...
static INLINE void read_coeffs_tx_intra_block(
    const AV2_COMMON *const cm, DecoderCodingBlock *dcb, aom_reader *const r,
...
static INLINE void decode_block_void(const AV2_COMMON *const cm,
...
static INLINE void predict_inter_block_void(AV2_COMMON *const cm,
                                            DecoderCodingBlock *dcb,
                                            BLOCK_SIZE bsize) {
  (void)cm;
  (void)dcb;
...
static INLINE void inverse_cross_chroma_transform_block(
    const AV2_COMMON *const cm, DecoderCodingBlock *dcb, aom_reader *const r,
...
static INLINE void decode_reconstruct_tx(AV2_COMMON *cm, ThreadData *const td,
                                         aom_reader *r,
...
static INLINE void set_offsets(AV2_COMMON *const cm, MACROBLOCKD *const xd,
                               BLOCK_SIZE bsize, int mi_row, int mi_col, int bw,
                               int bh, int x_inside_boundary,
                               int y_inside_boundary, PARTITION_TREE *parent,
                               int index) {
  const int num_planes = av2_num_planes(cm);
...
static void copy_tip_worker_data(AV2Decoder *pbi, AV2_COMMON *cm) {
  ...
  CHECK_MEM_ERROR(
      ...
      aom_memalign(32, num_worker * sizeof(*(pbi->tip_worker_data))));
...
static INLINE void decode_mbmi_block(AV2Decoder *const pbi,
...
static INLINE void dec_build_inter_predictor(const AV2_COMMON *cm,
                                             DecoderCodingBlock *dcb,
...
static INLINE void predict_inter_block(AV2_COMMON *const cm,
                                       DecoderCodingBlock *dcb,
                                       BLOCK_SIZE bsize) {
  MACROBLOCKD *const xd = &dcb->xd;
  MB_MODE_INFO *mbmi = xd->mi[0];
...
static INLINE void copy_frame_mvs_inter_block(AV2_COMMON *const cm,
                                              DecoderCodingBlock *dcb,
                                              BLOCK_SIZE bsize) {
  if (!frame_is_intra_only(cm) &&
...
static INLINE int bridge_frame_is_valid_inter(const AV2_COMMON *const cm,
...
static INLINE void decode_token_recon_block(AV2Decoder *const pbi,
...
static INLINE void parse_decode_block(AV2Decoder *const pbi,
                                      ThreadData *const td, int mi_row,
                                      int mi_col, aom_reader *r,
                                      PARTITION_TYPE partition,
                                      BLOCK_SIZE bsize, PARTITION_TREE *parent,
                                      int index) {
  DecoderCodingBlock *const dcb = &td->dcb;
  MACROBLOCKD *const xd = &dcb->xd;
...
static INLINE void set_offsets_for_pred_and_recon(AV2Decoder *const pbi,
                                                  ThreadData *const td,
                                                  int mi_row, int mi_col,
                                                  BLOCK_SIZE bsize) {
  AV2_COMMON *const cm = &pbi->common;
...
static INLINE void decode_block(AV2Decoder *const pbi, ThreadData *const td,
...
static INLINE void decode_partition(AV2Decoder *const pbi, ThreadData *const td,
...
static INLINE void decode_partition_sb(AV2Decoder *const pbi,
                                       ThreadData *const td, int mi_row,
...
static INLINE void setup_bru_active_info(AV2_COMMON *const cm,
                                         struct aom_read_bit_buffer *rb) {
  init_bru_params(cm);
...
static INLINE void setup_segmentation(AV2_COMMON *const cm,
                                      struct aom_read_bit_buffer *rb) {
...
static INLINE void decode_restoration_mode(AV2_COMMON *cm,
                                           struct aom_read_bit_buffer *rb) {
    ...
    if (is_wiener_nonsep_possible) {
      ...
      if (is_frame_filters_enabled(p)) {
        ...
        if (rsi->frame_filters_on) {
          ...
          if (num_ref_frames > 0) rsi->temporal_pred_flag = aom_rb_read_bit(rb);
...
static INLINE void setup_loopfilter(AV2_COMMON *cm,
                                    struct aom_read_bit_buffer *rb) {
  ...
  if (features->tip_frame_mode == TIP_FRAME_AS_OUTPUT) {
    if (cm->seq_params.enable_lf_sub_pu && features->allow_lf_sub_pu) {
      cm->lf.apply_deblocking_filter_tip = aom_rb_read_bit(rb);
  ...
  if (cm->mfh_params[cm->cur_mfh_id].mfh_deblocking_filter_update_flag)
    ...
    lf->apply_deblocking_filter[0] = aom_rb_read_bit(rb);
  if (cm->mfh_params[cm->cur_mfh_id].mfh_deblocking_filter_update_flag)
    lf->apply_deblocking_filter[1] =
        cm->mfh_params[cm->cur_mfh_id].mfh_apply_deblocking_filter[1];
  else
    lf->apply_deblocking_filter[1] = aom_rb_read_bit(rb);
  if (num_planes > 1) {
    if (lf->apply_deblocking_filter[0] || lf->apply_deblocking_filter[1]) {
      if (cm->mfh_params[cm->cur_mfh_id].mfh_deblocking_filter_update_flag) {
        ...
        lf->apply_deblocking_filter_u = aom_rb_read_bit(rb);
        lf->apply_deblocking_filter_v = aom_rb_read_bit(rb);
      }
    } else {
...
static INLINE void setup_gdf(AV2_COMMON *cm, struct aom_read_bit_buffer *rb) {
...
static INLINE void setup_cdef(AV2_COMMON *cm, struct aom_read_bit_buffer *rb) {
  const int num_planes = av2_num_planes(cm);
...
static INLINE int read_ccso_offset_idx(struct aom_read_bit_buffer *rb) {
  int offset_idx = 0;
...
static INLINE void setup_ccso(AV2_COMMON *cm, struct aom_read_bit_buffer *rb) {
  if (cm->bru.frame_inactive_flag || cm->bridge_frame_info.is_bridge_frame ||
      cm->features.tip_frame_mode == TIP_FRAME_AS_OUTPUT) {
...
static INLINE void setup_qm_params(AV2Decoder *pbi,
                                   CommonQuantParams *quant_params,
                                   bool segmentation_enabled, int num_planes,
                                   struct aom_read_bit_buffer *rb) {
...
static INLINE void setup_segmentation_dequant(AV2Decoder *const pbi,
                                              MACROBLOCKD *const xd) {
  AV2_COMMON *const cm = &pbi->common;
  ...
  for (int i = 0; i < max_segments; ++i) {
    ...
    for (int j = 0; j < TX_SIZES_ALL_AV2; ++j) {
      if (j > TX_8X8 && j != TX_4X8 && j != TX_8X4)
        ...
        quant_params->y_iqmatrix[i][j] =
            av2_iqmatrix(quant_params, qmlevel_y, AOM_PLANE_Y, j);
    }
    const int num_planes = av2_num_planes(cm);
    if (num_planes > 1) {
      const int qmlevel_u =
      ...
      for (int j = 0; j < TX_SIZES_ALL_AV2; ++j) {
        if (j > TX_8X8 && j != TX_4X8 && j != TX_8X4)
          ...
          quant_params->u_iqmatrix[i][j] =
              av2_iqmatrix(quant_params, qmlevel_u, AOM_PLANE_U, j);
      }
      const int qmlevel_v =
      ...
      for (int j = 0; j < TX_SIZES_ALL_AV2; ++j) {
        if (j > TX_8X8 && j != TX_4X8 && j != TX_8X4)
          ...
          quant_params->v_iqmatrix[i][j] =
              av2_iqmatrix(quant_params, qmlevel_v, AOM_PLANE_V, j);
      }
#if CONFIG_QM_DEBUG
...
static INLINE void setup_render_size(AV2_COMMON *cm,
                                     struct aom_read_bit_buffer *rb) {
  if (cm->bridge_frame_info.is_bridge_frame) {
    return;
  }
...
static INLINE void resize_context_buffers(AV2_COMMON *cm, int width,
                                          int height) {
...
static INLINE void setup_tip_frame_size(AV2_COMMON *cm) {
  const SequenceHeader *const seq_params = &cm->seq_params;
  ...
  if (tip_frame_buf) {
    ...
    if (ci_params->ci_chroma_sample_position_present_flag)
      ...
      tip_frame_buf->chroma_sample_position = AOM_CSP_UNKNOWN;
...
static INLINE void setup_buffer_pool(AV2_COMMON *cm) {
  ...
  if (ci_params->ci_chroma_sample_position_present_flag)
    ...
    cm->cur_frame->buf.chroma_sample_position = AOM_CSP_UNKNOWN;
...
static INLINE void setup_frame_size(AV2_COMMON *cm,
...
static INLINE void setup_seq_sb_size(SequenceHeader *seq_params,
                                     struct aom_read_bit_buffer *rb) {
...
static INLINE void setup_frame_size_with_refs(AV2_COMMON *cm,
...
static INLINE void read_tile_info_max_tile(
    AV2_COMMON *const cm, struct aom_read_bit_buffer *const rb) {
  CommonTileParams *const tiles = &cm->tiles;
...
static INLINE void read_tile_info(AV2Decoder *const pbi,
                                  struct aom_read_bit_buffer *const rb) {
  AV2_COMMON *const cm = &pbi->common;
  if (cm->bridge_frame_info.is_bridge_frame) {
    return;
...
static INLINE void set_cb_buffer(AV2Decoder *pbi, DecoderCodingBlock *dcb,
...
static INLINE void decoder_alloc_tile_data(AV2Decoder *pbi, const int n_tiles) {
  AV2_COMMON *const cm = &pbi->common;
...
static INLINE void dec_row_mt_alloc(AV2DecRowMTSync *dec_row_mt_sync,
...
static INLINE void decode_tile_sb_row(AV2Decoder *pbi, ThreadData *const td,
...
static INLINE void decode_tile(AV2Decoder *pbi, ThreadData *const td,
...
static INLINE void tile_worker_hook_init(AV2Decoder *const pbi,
...
static INLINE void parse_tile_row_mt(AV2Decoder *pbi, ThreadData *const td,
                                     TileDataDec *const tile_data) {
  AV2_COMMON *const cm = &pbi->common;
  const int sb_mi_size = mi_size_wide[cm->sb_size];
  const int num_planes = av2_num_planes(cm);
...
static INLINE void enqueue_tile_jobs(AV2Decoder *pbi, AV2_COMMON *cm,
...
static INLINE void alloc_dec_jobs(AV2DecTileMT *tile_mt_info, AV2_COMMON *cm,
...
// processing.
static INLINE void allocate_opfl_tmp_bufs(AV2_COMMON *const cm,
...
static INLINE void allocate_mc_tmp_buf(AV2_COMMON *const cm,
...
static INLINE void tile_mt_queue(AV2Decoder *pbi, int tile_cols, int tile_rows,
                                 int tile_rows_start, int tile_rows_end,
                                 int tile_cols_start, int tile_cols_end,
                                 int start_tile, int end_tile) {
  AV2_COMMON *const cm = &pbi->common;
...
static INLINE void dec_alloc_cb_buf(AV2Decoder *pbi) {
  AV2_COMMON *const cm = &pbi->common;
...
static INLINE void row_mt_frame_init(AV2Decoder *pbi, int tile_rows_start,
...
int av2_get_bitdepth_from_index(uint32_t bitdepth_lut_idx) {
  static aom_bit_depth_t bitdepth_list[] = { AOM_BITS_10, AOM_BITS_8,
                                             AOM_BITS_12 };
  if (bitdepth_lut_idx >= AV2_NUM_SUPPORTED_BITDEPTH) return -1;
...
static void setup_film_grain(AV2Decoder *pbi, struct aom_read_bit_buffer *rb) {
  AV2_COMMON *const cm = &pbi->common;
  const SequenceHeader *const seq_params = &cm->seq_params;
...
static void set_seq_chroma_format(uint32_t seq_chroma_format_idc,
                                  SequenceHeader *seq_params,
                                  struct aom_internal_error_info *error_info) {
  aom_codec_err_t err = av2_get_chroma_subsampling(seq_chroma_format_idc,
...
void av2_read_conformance_window(struct aom_read_bit_buffer *rb,
...
void read_sequence_inter_group_tool_flags(struct SequenceHeader *seq_params,
  ...
  if (seq_params->single_picture_header_flag) {
    seq_params->enable_opfl_refine = AV2_OPFL_REFINE_NONE;
...
void read_sequence_filter_group_tool_flags(struct SequenceHeader *seq_params,
  ...
  if (seq_params->enable_gdf && seq_params->sb_size != BLOCK_256X256 &&
    ...
    seq_params->gdf_unit_matches_sb_size = aom_rb_read_bit(rb);
  } else {
  ...
  if (seq_params->enable_ccso) {
    seq_params->ccso_unit_matches_sb_size = aom_rb_read_bit(rb);
  } else {
...
void av2_read_sequence_header(struct aom_read_bit_buffer *rb,
                              SequenceHeader *seq_params) {
...
static INLINE void read_multi_frame_header_seg_info(
...
uint32_t av2_read_multi_frame_header(AV2_COMMON *cm,
  ...
  const uint32_t mfh_id_minus_1 = aom_rb_read_uvlc(rb);
  ...
  mfh_param->mfh_deblocking_filter_update_flag = aom_rb_read_bit(rb);
  if (mfh_param->mfh_deblocking_filter_update_flag) {
    for (int i = 0; i < 4; i++) {
      mfh_param->mfh_apply_deblocking_filter[i] = aom_rb_read_bit(rb);
    }
  } else {
    for (int i = 0; i < 4; i++) {
...
static INLINE void read_global_motion(AV2_COMMON *cm,
                                      struct aom_read_bit_buffer *rb) {
  const SequenceHeader *const seq_params = &cm->seq_params;
...
static INLINE void reset_ref_frame_map(AV2_COMMON *const cm) {
  BufferPool *const pool = cm->buffer_pool;
...
static INLINE void validate_references(AV2Decoder *const pbi) {
  AV2_COMMON *const cm = &pbi->common;
...
static int read_show_existing_frame(AV2Decoder *pbi, bool is_regular_obu,
  ...
  if (!cm->derive_sef_order_hint) {
    ...
    setup_buffer_pool(cm);
    aom_yv12_copy_frame(&frame_to_show->buf, &cm->cur_frame->buf,
                        av2_num_planes(cm));
  } else {
...
static void read_frame_opfl_refine_type(AV2_COMMON *const cm,
  ...
  if (cm->features.tip_frame_mode == TIP_FRAME_AS_OUTPUT) {
    cm->features.opfl_refine_type =
        (cm->seq_params.enable_opfl_refine == AV2_OPFL_REFINE_NONE ||
    ...
    for (int ref_mlayer_id = 0; ref_mlayer_id < cur_mlayer_id;
      ...
      if (seq_header->mlayer_dependency_map[cur_mlayer_id][ref_mlayer_id] ==
        ...
        if ((mlayer_map & (1 << cur_mlayer_id)) &&
          ...
          aom_internal_error(
              info, AOM_CODEC_UNSUP_BITSTREAM,
...
static int setup_multiframe_header_id(AV2_COMMON *const cm, OBU_TYPE obu_type,
                                      struct aom_read_bit_buffer *rb) {
  uint32_t cur_mfh_id = obu_type == OBU_BRIDGE_FRAME ? 0 : aom_rb_read_uvlc(rb);
...
static int setup_sequence_header_id(AV2_COMMON *const cm,
  ...
  if (cm->cur_mfh_id == 0) {
    seq_header_id_in_frame_header = aom_rb_read_uvlc(rb);
...
static int read_uncompressed_header(AV2Decoder *pbi, OBU_TYPE obu_type,
  ...
  if (seq_params->single_picture_header_flag) {
    ...
    if (obu_type == OBU_CLOSED_LOOP_KEY || obu_type == OBU_OPEN_LOOP_KEY) {
      ...
      cm->restricted_prediction_switch = aom_rb_read_bit(rb);
      ...
      current_frame->frame_type =
          aom_rb_read_bit(rb) ? INTER_FRAME : INTRA_ONLY_FRAME;
    }
    ...
    if (obu_type == OBU_RAS_FRAME || obu_type == OBU_OPEN_LOOP_KEY) {
      ...
      if (seq_params->number_of_bits_for_lt_frame_id != 0) {
        ...
        for (int i = 0; i < cm->num_ref_key_frames; i++) {
          cm->ref_long_term_ids[i] = aom_rb_read_literal(
    ...
    if (cm->bridge_frame_info.is_bridge_frame) {
      ...
      if (obu_type == OBU_OPEN_LOOP_KEY)
        ...
        cm->immediate_output_picture = aom_rb_read_bit(rb);
    }
    ...
    if (!cm->immediate_output_picture) {
      if (cm->bridge_frame_info.is_bridge_frame) {
        ...
        cm->implicit_output_picture = aom_rb_read_bit(rb);
      } else {
        cm->implicit_output_picture = 0;
      }
    } else {
...
struct aom_read_bit_buffer *av2_init_read_bit_buffer(
    AV2Decoder *pbi, struct aom_read_bit_buffer *rb, const uint8_t *data,
    const uint8_t *data_end) {
  rb->bit_offset = 0;
  rb->error_handler = (aom_rb_error_handler)error_handler;
...
void av2_read_frame_size(struct aom_read_bit_buffer *rb, int num_bits_width,
                         int num_bits_height, int *width, int *height) {
  *width = aom_rb_read_literal(rb, num_bits_width) + 1;
  *height = aom_rb_read_literal(rb, num_bits_height) + 1;
}
BITSTREAM_PROFILE av2_read_profile(struct aom_read_bit_buffer *rb) {
  int profile = aom_rb_read_literal(rb, PROFILE_BITS);
  return (BITSTREAM_PROFILE)profile;
}
static INLINE void tip_mode_legal_check(AV2Decoder *const pbi) {
  AV2_COMMON *const cm = &pbi->common;
...
static INLINE void process_tip_mode(AV2Decoder *pbi) {
  AV2_COMMON *const cm = &pbi->common;
  const int num_planes = av2_num_planes(cm);
  MACROBLOCKD *const xd = &pbi->dcb.xd;
...
static INLINE void setup_frame_info(AV2Decoder *pbi) {
  AV2_COMMON *const cm = &pbi->common;
