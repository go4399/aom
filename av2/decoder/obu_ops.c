...
uint32_t av2_read_operating_point_set_obu(struct AV2Decoder *pbi,
  ...
  if (ops->ops_cnt > 0) {
    ...
    for (int i = 0; i < ops->ops_cnt; i++) {
      ...
      op->ops_data_size = aom_rb_read_uleb(rb);
      ...
      if (ops->ops_color_info_present_flag) {
        ...
        op->color_info.ops_color_description_idc = AV2_COLOR_DESC_IDC_EXPLICIT;
        op->color_info.ops_color_primaries = AOM_CICP_CP_UNSPECIFIED;
        op->color_info.ops_transfer_characteristics = AOM_CICP_TC_UNSPECIFIED;
