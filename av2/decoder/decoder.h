...
typedef struct AV2Decoder {
  ...
  RefCntBuffer
      *output_frames[(REF_FRAMES + 1) *
                     AV2_MAX_NUM_STREAMS];  // RefCntBuffer is used for a single
   ...
   */
  int xlayer_id_map[AV2_MAX_NUM_STREAMS];
  /*!
   * Map to indicate which mlayer is present in the current CVS.
   */
  int mlayer_id_map[AV2_MAX_NUM_STREAMS][MAX_NUM_MLAYERS];
...
aom_codec_err_t av2_copy_reference_dec(struct AV2Decoder *pbi, int idx,
