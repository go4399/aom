  ...
  *color_description_idc = avm_rb_read_rice_golomb(rb, 2);
  ...
  switch (*color_description_idc) {
    ...
    case AV2_COLOR_DESC_IDC_BT709SDR:                  // 1
      ...
      *transfer_characteristics = AOM_CICP_TC_BT_709;  // 1
      *matrix_coefficients = AOM_CICP_MC_BT_709;       // 1
      break;
    case AV2_COLOR_DESC_IDC_BT2100PQ:                      // 2
      *color_primaries = AOM_CICP_CP_BT_2020;              // 9
      *transfer_characteristics = AOM_CICP_TC_SMPTE_2084;  // 16
      *matrix_coefficients = AOM_CICP_MC_BT_2020_NCL;      // 9
      break;
    case AV2_COLOR_DESC_IDC_BT2100HLG:                 // 3
      *color_primaries = AOM_CICP_CP_BT_2020;          // 9
      *transfer_characteristics = AOM_CICP_TC_HLG;     // 18
      *matrix_coefficients = AOM_CICP_MC_BT_2020_NCL;  // 9
      break;
    case AV2_COLOR_DESC_IDC_SRGB:                    // 4
      *color_primaries = AOM_CICP_CP_BT_709;         // 1
      *transfer_characteristics = AOM_CICP_TC_SRGB;  // 13
      *matrix_coefficients = AOM_CICP_MC_IDENTITY;   // 0
      break;
    case AV2_COLOR_DESC_IDC_SYCC:                     // 5
      *color_primaries = AOM_CICP_CP_BT_709;          // 1
      *transfer_characteristics = AOM_CICP_TC_SRGB;   // 13
      *matrix_coefficients = AOM_CICP_MC_BT_470_B_G;  // 5
      break;
    default:
...
aom_codec_err_t flush_all_xlayer_frames(struct AV2Decoder *pbi, AV2_COMMON *cm,
                                        bool release_dpb) {
  BufferPool *const pool = cm->buffer_pool;
  int saved_xlayer_id = cm->xlayer_id;
  aom_codec_err_t err = AOM_CODEC_OK;

  for (int xlayer = 0; xlayer < AV2_MAX_NUM_STREAMS; xlayer++) {
...
static uint32_t read_multi_stream_decoder_operation_obu(
    AV2Decoder *pbi, struct aom_read_bit_buffer *rb) {
  ...
  if (pbi->stream_info != NULL && config_changed) {
    ...
    for (int i = 0; i < AV2_MAX_NUM_STREAMS; i++) {
  ...
  if (pbi->stream_info == NULL) {
    pbi->stream_info =
        (StreamInfo *)aom_malloc(num_streams * sizeof(StreamInfo));
    if (pbi->stream_info == NULL) {
...
static void skip_remaining_mu_payload_bits(struct aom_read_bit_buffer *rb,
...
static size_t read_metadata_unit_header(AV2Decoder *pbi, const uint8_t *data,
  ...
  if (!metadata->cancel_flag) {
    ...
    if (aom_uleb_decode(data + bytes_read, total_size - bytes_read, &size_value,
                        &size_length) < 0) {
      cm->error.error_code = AOM_CODEC_CORRUPT_FRAME;
      return 0;
    }
...
static void check_tilegroup_obus_in_a_frame_unit(AV2_COMMON *const cm,
  ...
  if (current_obu->obu_type != prev_obu->obu_type ||
    ...
    aom_internal_error(
        ...
        __func__, aom_obu_type_to_string(current_obu->obu_type),
        current_obu->display_order_hint,
        aom_obu_type_to_string(prev_obu->obu_type),
...
static void check_clk_in_a_layer(AV2_COMMON *const cm,
  ...
  if (current_frame_unit->obu_type == OBU_CLOSED_LOOP_KEY &&
    ...
    aom_internal_error(
        ...
        __func__, aom_obu_type_to_string(current_frame_unit->obu_type),
        current_frame_unit->display_order_hint, current_frame_unit->mlayer_id,
        aom_obu_type_to_string(last_frame_unit->obu_type),
  ...
  if (last_frame_unit->showable_frame == 0 &&
    ...
    if (current_frame_unit->display_order_hint ==
      ...
      aom_internal_error(
          ...
          aom_obu_type_to_string(last_frame_unit->obu_type),
          last_frame_unit->showable_frame,
          aom_obu_type_to_string(last_displayable_frame_unit->obu_type),
          last_displayable_frame_unit->display_order_hint,
          last_displayable_frame_unit->mlayer_id);
    }
...
static void check_valid_layer_id(ObuHeader obu_header, AV2_COMMON *const cm) {
  ...
  if (obu_header.obu_xlayer_id == GLOBAL_XLAYER_ID &&
    ...
    aom_internal_error(
        ...
        aom_obu_type_to_string(obu_header.type), obu_header.obu_tlayer_id,
...
bool conformance_check_msdo_lcr(struct AV2Decoder *pbi, bool global_lcr_present,
  ...
  for (int i = 0; i < AV2_MAX_NUM_STREAMS - 1; i++) {
    if (pbi->xlayer_id_map[i] > 0) num_extended_layers++;
  }
  for (int i = 0; i < MAX_NUM_MLAYERS; i++) {
    bool found = false;
    for (int x = 0; x < AV2_MAX_NUM_STREAMS; x++) {
...
int avm_decode_frame_from_obus(struct AV2Decoder *pbi, const uint8_t *data,
  ...
  if (pbi->is_multistream) {
    ...
    if (tid >= 0 && mid >= 0 &&
      ...
      if (!pbi->prescan_glcr_will_activate) {
        ...
        cm->num_streams = 0;
        for (int i = 0; i < AV2_MAX_NUM_STREAMS; i++) pbi->xlayer_id_map[i] = 0;
  ...
  if (pbi->random_accessed) {
    ...
    for (int i = 0; i < AV2_MAX_NUM_STREAMS - 1; i++) pbi->xlayer_id_map[i] = 0;
    for (int x = 0; x < AV2_MAX_NUM_STREAMS; x++)
  ...
  while (!frame_decoding_finished && cm->error.error_code == AOM_CODEC_OK) {
    ...
    if (av2_obu_type_is_valid(obu_header.type) &&
        obu_header.type != OBU_PADDING) {
      if (!check_temporal_unit_structure(
        ...
        aom_internal_error(
            ...
            aom_obu_type_to_string(obu_header.type), obu_header.obu_xlayer_id,
            obu_header.obu_mlayer_id, aom_obu_type_to_string(prev_obu_type),
            prev_xlayer_id);
      }
    ...
    switch (obu_header.type) {
      case OBU_TEMPORAL_DELIMITER:
        ...
        for (int xlayer = 0; xlayer < AV2_MAX_NUM_STREAMS - 1; xlayer++) {
          if (pbi->xlayer_id_map[xlayer] > 0) {
            av2_store_xlayer_context(pbi, cm, cm->xlayer_id);
            cm->xlayer_id = xlayer;
            av2_restore_xlayer_context(pbi, cm, xlayer);
