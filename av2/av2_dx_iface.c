...
static void bitreader_set_failed(void *data) {
...
static aom_codec_err_t decoder_decode(aom_codec_alg_priv_t *ctx,
  ...
  if (ctx->frame_worker) {
    ...
    if (data == NULL && data_sz == 0) {
      AV2_COMMON *const cm = &pbi->common;
      aom_codec_err_t err = flush_all_xlayer_frames(pbi, cm, false);
...
static void move_decoder_metadata_to_img(AV2Decoder *pbi, aom_image_t *img) {
  if (pbi->metadata && img) {
    assert(!img->metadata);
...
static void copy_frame_hash_metadata_to_img(
    aom_image_t *img, RefCntBuffer *const output_frame_buf) {
...
static aom_codec_err_t ctrl_get_s_frame_info(aom_codec_alg_priv_t *ctx,
                                             va_list args) {
  av2_s_frame_info *const s_frame_info = va_arg(args, av2_s_frame_info *);
  if (s_frame_info) {
    if (ctx->frame_worker) {
      AVxWorker *const worker = ctx->frame_worker;
      FrameWorkerData *const frame_worker_data =
...
static aom_codec_err_t ctrl_set_random_access(aom_codec_alg_priv_t *ctx,
                                              va_list args) {
...
static aom_codec_err_t ctrl_set_selected_ops(aom_codec_alg_priv_t *ctx,
                                             va_list args) {
...
static aom_codec_err_t ctrl_set_sub_bitstream_extraction(
    aom_codec_alg_priv_t *ctx, va_list args) {
  ctx->enable_sub_bitstream_extraction = va_arg(args, int);
  return AOM_CODEC_OK;
}

static aom_codec_err_t ctrl_set_selected_local_ops(aom_codec_alg_priv_t *ctx,
                                                   va_list args) {
...
const aom_codec_iface_t *avm_codec_av2_dx(void) { return &avm_codec_av2_dx_algo_temp; }
