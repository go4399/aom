...
The extended cq-level range (64–80) is supported **only** in constant quality mode (`AOM_Q`) with `use-fixed-qp-offsets=1` and TPL disabled (`enable-tpl-model=0`):
- **`AOM_Q` + `use-fixed-qp-offsets=1` + `enable-tpl-model=0`**: Uses qstep_ratio approach in `encoder_utils.c` — covered in Part 1
- **All other modes/settings**: cq-level > 63 is rejected with an assertion and error message — covered in Part 2
...
### Verified qindex mapping (8-bit, `use-fixed-qp-offsets=1`)

Values verified against actual encoder output (FourPeople 480x270, 130 frames, cpu-used=4, 8-bit). KF and ARF produce identical qindex in fixed QP mode. At cq <= 79, KF/ARF shows off-by-1 from predicted values due to DC quantizer rounding in `av1_get_q_index_from_qstep_ratio()`; IARF and Leaf are exact. At cq=80, the `qstep_ratio >= 1.0` shortcut ensures all layers reach exactly 255.

| cq | qindex | KF/ARF | IARF_d2 | IARF_d3 | IARF_d4 | Leaf |
|----|--------|--------|---------|---------|---------|------|
| 0  | 0      | 0      | 0       | 0       | 0       | 0    |
| 10 | 40     | 13     | 27      | 34      | 37      | 40   |
| 20 | 80     | 27     | 54      | 67      | 74      | 80   |
| 30 | 120    | 44     | 82      | 101     | 111     | 120  |
| 40 | 160    | 74     | 117     | 139     | 150     | 160  |
| 50 | 200    | 106    | 153     | 177     | 189     | 200  |
| 55 | 220    | 121    | 171     | 196     | 208     | 220  |
| 60 | 240    | 142    | 191     | 216     | 228     | 240  |
| 63 | 255    | 172    | 214     | 235     | 245     | 255  |
| 64 | 259    | 188    | 222     | 239     | 247     | 255  |
| 66 | 267    | 211    | 233     | 244     | 250     | 255  |
| 68 | 275    | 226    | 241     | 248     | 252     | 255  |
| 70 | 283    | 235    | 245     | 250     | 253     | 255  |
| 72 | 291    | 241    | 248     | 252     | 254     | 255  |
| 74 | 299    | 246    | 251     | 253     | 254     | 255  |
| 76 | 307    | 249    | 252     | 254     | 255     | 255  |
| 78 | 315    | 252    | 254     | 255     | 255     | 255  |
| 79 | 319    | 253    | 254     | 255     | 255     | 255  |
...
Add conditional assertion after the range check — reject cq > 63 unless AOM_Q + fixed QP + TPL disabled:
```c
...
## Part 2: Guard for Unsupported Modes

### Approach: Assert and Reject

For any configuration other than `AOM_Q` + `use-fixed-qp-offsets=1` + `enable-tpl-model=0`, cq-level values above 63 are not supported. The encoder validates this at configuration time and rejects invalid combinations with a clear error message.

The validation logic is added in `av1_cx_iface.c` (see Part 1, item 4 above).

### Rationale

The non-fixed QP path (`use-fixed-qp-offsets=0`) uses polynomial minq table lookups and `av1_compute_qdelta` with content-adaptive state (`avg_frame_qindex`). Extending virtual qindex values into this path introduces inconsistencies — for example, the first ARF frame gets qindex=0 due to boost-table lookups (`get_gf_active_quality()`) that ignore `cq_level` and rely on `avg_frame_qindex`, which is near-zero early in the sequence. Rather than patching multiple rate control code paths with workarounds, the extended range is restricted to the deterministic fixed-QP path where qindex assignment is fully controlled by the qstep_ratio formula.

When TPL is enabled (`enable-tpl-model=1`), the temporal propagation model can override the fixed QP offsets, making the qstep_ratio-based qindex assignment non-deterministic. The extended cq-level range relies on the deterministic fixed-QP path, so TPL must be disabled.

Similarly, CQ/CBR/VBR modes either don't use `cq_level` or use it only for comparisons that assume qindex ≤ 255. Extending into those modes offers no benefit and risks subtle breakage.
...
| **AOM_CQ**       | Yes (comparisons) | cq_level > 63 rejected at config validation                        | GUARDED        |
| **AOM_Q (fixed QP)**| Yes (direct) | Extended range handled by qstep_ratio ramp + shortcut              | SAFE with fixes |
| **AOM_Q (non-fixed)**| Yes (direct) | cq_level > 63 rejected at config validation                        | GUARDED        |
...
- Config validation rejects cq > 63 for any mode other than AOM_Q + `use-fixed-qp-offsets=1` + `enable-tpl-model=0`
...
| `av1/av1_cx_iface.c`         | cq_level range 63→80; clamp best/worst_allowed_q; assert cq>63 requires AOM_Q + fixed-qp-offsets | Done   |
| `av1/encoder/ratectrl.c`     | Clamp qindex in `av1_convert_qindex_to_q()`                   | Done   |
...
| Test | Command | Expected Result | Status |
|------|---------|-----------------|--------|
| Non-fixed QP + extended cq | `--end-usage=q --use-fixed-qp-offsets=0 --cq-level=70` | Rejected: "cq-level > 63 requires use-fixed-qp-offsets=1" | — |
| TPL enabled + extended cq | `--end-usage=q --use-fixed-qp-offsets=1 --enable-tpl-model=1 --cq-level=70` | Rejected: "cq-level > 63 requires enable-tpl-model=0" | — |
| CQ mode + extended cq | `--end-usage=cq --cq-level=70` | Rejected: "cq-level > 63 is only supported in AOM_Q mode" | — |
| CBR + extended cq | `--end-usage=cbr --cq-level=70` | Rejected: "cq-level > 63 is only supported in AOM_Q mode" | — |
| VBR + extended cq | `--end-usage=vbr --cq-level=70` | Rejected: "cq-level > 63 is only supported in AOM_Q mode" | — |
| Non-fixed QP + original range | `--end-usage=q --use-fixed-qp-offsets=0 --cq-level=32` | No regression | — |
...
| CQ mode original range | `--end-usage=cq --cq-level=50` | Runs correctly | Pass |
